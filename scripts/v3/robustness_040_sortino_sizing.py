"""Robustness diagnostics for family 040's primary configuration (plan sec
4.3). Modeled on scripts/v3/robustness_031_kelly_sharpe_sizing.py: rolling
...
array m_t (derived from the percentile-ranked trailing Sortino ratio),
NOT the raw price series -- circular-shifting m_t tests whether the
specific temporal alignment of the Sortino-percentile signal (not just
...
def strat_builder(daily):
    return sos.make_sortino_sizing_decider(daily, WEEKLY_DEPOSIT, **sos.PRIMARY_CONFIG)
...
def main():
    ...
    m_real = sos.compute_multiplier(daily, cfg["sortino_window"], cfg["pctile_lookback"], cfg["k"], cfg["min_mult"])
    ...
    def strategy_decide_from_signal(shifted_m):
        lump_cap = sos.MAX_LUMP_MULTIPLE * WEEKLY_DEPOSIT
