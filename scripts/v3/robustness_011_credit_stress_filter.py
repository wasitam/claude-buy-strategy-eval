"""Robustness diagnostics for family 011's primary configuration (plan sec
4.3). Modeled directly on scripts/v3/robustness_005_tsmom_sizing.py: rolling
windows (3y/5y for SP500/GOLD/SILVER/OIL, 2y for BTC), block bootstrap
raw+detrended, and placebo circular-shift -- at n_sims=60, the time-budget
precedent established for this loop starting with family 010 (only run in
full if sec 4.1 passes, per this family's task instruction and the
005/010 60-sim convention).

Placebo signal here is the primary config's own `stressed` boolean regime
array (SP500). Circular-shifting it and rerunning via
make_decider_from_stressed_array tests whether the SPECIFIC temporal
alignment of the credit-stress regime (not just spending some fraction of
weeks banking cash) is doing the work.
"""
...
def strat_builder(daily):
    return csf.make_credit_stress_decider(daily, WEEKLY_DEPOSIT, **csf.PRIMARY_CONFIG)
...
def main():
    ...
    # --- placebo: circular-shift the primary config's own stressed regime array, SP500 ---
    stressed_real = csf.compute_stressed(
        daily, csf.PRIMARY_CONFIG["signal_choice"], csf.PRIMARY_CONFIG["lookback_years"],
        csf.PRIMARY_CONFIG["stress_pctile"],
    )
    ...
    def strategy_decide_from_signal(shifted_stressed):
        return csf.make_decider_from_stressed_array(
            daily, WEEKLY_DEPOSIT, shifted_stressed,
            stress_tilt_fraction=csf.PRIMARY_CONFIG["stress_tilt_fraction"],
            max_lump_multiple=csf.PRIMARY_CONFIG["max_lump_multiple"],
        )

    df_pl, pct_w, pct_s = v3rob.placebo_circular_shift(
        daily, rf, WEEKLY_DEPOSIT, stressed_real, strategy_decide_from_signal, bench_builder,
