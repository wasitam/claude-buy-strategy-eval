"""Run family 004 (value averaging) end-to-end on development data:
implementation checks -> full grid -> trial logging -> DSR/N_eff -> assess.
Writes state/trials/new_004_*.csv, updates state/trial_counter.json, and
dumps results into families/004-value-averaging/_run_output.json for the
results.md writer. Modeled directly on scripts/v3/run_003_vol_managed_sizing.py
(same single-asset pattern).
"""
...
def decider_builder_factory(daily, fee, annual_growth_rate, max_multiple_of_deposit, allow_sells):
    def builder(enabled=True):
        return va.make_value_averaging_decider(
            daily, WEEKLY_DEPOSIT, annual_growth_rate=annual_growth_rate,
            max_multiple_of_deposit=max_multiple_of_deposit, allow_sells=allow_sells,
            fee=fee, enabled=enabled,
        )
    return builder
...
def run_impl_checks(dev):
    ...
    rf = dev["rf"]
    builder = decider_builder_factory(daily, 0.001, **va.PRIMARY_CONFIG)
    ...
    decide_va = builder(enabled=True)
    res_va = v3eng.run_single_asset(daily, WEEKLY_DEPOSIT, rf, decide_va, fee=0.001).to_frame()
    out["no_negative_cash_units_value_averaging"] = v3chk.check_no_negative_cash_or_units(res_va)

    # Capital-neutrality check: cumulative net capital deployed (buys - sell
    # proceeds) must never exceed what the SAME $500/week deposit stream would
    # be worth if it had just sat in cash earning IRX the whole time (i.e. no
    # external capital is ever injected -- every dollar VA deploys traces back
    # to this account's own deposits + the interest they themselves earned
    # while un-invested). This is a stricter, path-wise version of "cumulative
    # deposits" that correctly credits organic interest on the cash buffer
    # (which the engine's own no-negative-cash invariant already guarantees
    # cannot be exceeded, since buys are always capped by real cash on hand;
    # this check makes that guarantee explicit and asset-agnostic).
    cash_only = np.zeros(len(res_va))
    rf_arr = rf.reindex(res_va.index).fillna(0.0).to_numpy()
    dep_arr = res_va["deposit"].to_numpy()
    c = 0.0
    for i in range(len(res_va)):
        c *= 1.0 + rf_arr[i]
        c += dep_arr[i]
        cash_only[i] = c
    net_deployed = (res_va["buy_usd"] - res_va["sell_usd"]).cumsum().to_numpy()
    out["never_exceeds_dca_cumulative_deposits_plus_interest"] = bool((net_deployed <= cash_only + 1e-6).all())
...
def run_one(daily, rf, cfg, fee):
    decide = va.make_value_averaging_decider(daily, WEEKLY_DEPOSIT, fee=fee, **cfg)
...
def cfg_id_for(ci, cfg):
    sells = "T" if cfg["allow_sells"] else "F"
    return f"cfg{ci:02d}_g{cfg['annual_growth_rate']}_k{cfg['max_multiple_of_deposit']}_s{sells}"
...
def main():
    ...
    primary_ci = configs.index(va.PRIMARY_CONFIG)
    primary_cfg_id = cfg_id_for(primary_ci, va.PRIMARY_CONFIG)
    primary_trial_id = f"new_004_{primary_cfg_id}"
