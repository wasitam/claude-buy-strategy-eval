"""Run family 038 (consumer-sentiment contrarian regime) end-to-end on
development data: implementation checks -> pre-grid sanity checks -> full
grid -> trial logging -> DSR/N_eff -> assess. Writes
state/trials/new_038_*.csv, updates state/trial_counter.json, and dumps
results into families/038-consumer-sentiment-contrarian/_run_output.json
...
pattern), adapted for UMCSENT's ABOVE-threshold (contrarian-on-euphoria)
trigger direction and its own known-episode check.
"""
...
from src.backtest.v3.strategies import consumer_sentiment_regime as csr

ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
STATE_DIR = os.path.join(ROOT, "state")
TRIALS_DIR = os.path.join(STATE_DIR, "trials")
FAM_DIR = os.path.join(ROOT, "families", "038-consumer-sentiment-contrarian")
FEES = [0.001, 0.0025]
...
def decider_builder_factory(daily, cfg):
    def builder(enabled=True):
        return csr.make_sentiment_regime_decider(daily, WEEKLY_DEPOSIT, enabled=enabled, **cfg)
    return builder


def run_sanity_checks(dev):
    """Pre-grid sanity checks (prereg.md): (1) non-degeneracy -- the primary
    config's euphoric condition fires non-trivially on every core asset;
    (2) known-episode check -- the Jan 2000 dot-com-era UMCSENT peak must
    read as euphoric (NOT normal), strictly within dev dates."""
    cfg = csr.PRIMARY_CONFIG
    rows = []
    for asset in ASSETS:
        daily = dev["prices"][asset]
        euphoric = csr.compute_euphoric(daily, cfg["normalization_method"], cfg["lookback_years"], cfg["threshold_level"])
        frac_euphoric = float(euphoric.mean())
        rows.append({"asset": asset, "dev_days": len(euphoric), "euphoric_days": int(euphoric.sum()), "frac_euphoric": frac_euphoric})

    # Known-episode check: Jan 2000 dot-com-era UMCSENT peak (~112.0, an
    # all-time high), checked on the signal itself (asset-agnostic), well
    # within dev dates (pre-2020).
    euphoric_bool, stat = csr._euphoric_at_signal_dates(
        cfg["normalization_method"], cfg["lookback_years"], cfg["threshold_level"]
    )
    window = euphoric_bool.loc["1999-06-01":"2000-05-31"]
    stat_window = stat.loc["1999-06-01":"2000-05-31"]
    episode = {
        "window": "1999-06-01..2000-05-31",
        "n_obs": int(len(window)),
        "frac_euphoric_in_window": float(window.mean()) if len(window) else float("nan"),
        "stat_at_window_start": float(stat_window.iloc[0]) if len(stat_window) else float("nan"),
        "stat_at_window_end": float(stat_window.iloc[-1]) if len(stat_window) else float("nan"),
        "max_stat_in_window": float(stat_window.max()) if len(stat_window) else float("nan"),
    }
    return rows, episode
...
def run_impl_checks(dev):
    ...
    # documented 35-day lag -- proving the lag actually matters (not a
    # no-op) -- while every real backtest below uses the documented lag
    # only.
    euphoric_documented = csr.compute_euphoric(daily, cfg["normalization_method"], cfg["lookback_years"], cfg["threshold_level"])
    euphoric_zero_lag = csr.compute_euphoric(daily, cfg["normalization_method"], cfg["lookback_years"], cfg["threshold_level"], lag_override_days=0)
    frac_diff = float((euphoric_documented != euphoric_zero_lag).mean())
...
def run_one(daily, rf, cfg, fee):
    decide = csr.make_sentiment_regime_decider(daily, WEEKLY_DEPOSIT, **cfg)
...
def cfg_id_for(ci, cfg):
    return (f"cfg{ci:02d}_{cfg['normalization_method']}_lb{cfg['lookback_years']}"
            f"_tl{cfg['threshold_level']}_tf{cfg['euphoric_tilt_fraction']}")
...
def main():
    ...
    for row in sanity:
        if not (0.02 <= row["frac_euphoric"] <= 0.98):
            raise SystemExit(f"Sanity check FAILED for {row['asset']}: frac_euphoric={row['frac_euphoric']:.4f} "
                              "outside (2%, 98%) -- degenerate regime, stopping before grid.")
    if episode["n_obs"] == 0:
        raise SystemExit("Known-episode check FAILED: no 1999-06..2000-05 UMCSENT observations found in dev data.")
    if episode["frac_euphoric_in_window"] < 0.5:
        raise SystemExit(
            f"Known-episode check FAILED: the primary config reads only "
            f"{episode['frac_euphoric_in_window']:.1%} of the Jan-2000 dot-com-era window as EUPHORIC; "
            "expected a clear majority -- signal construction does not match the well-known episode."
        )
    print(f"Known-episode check PASSED: 1999-06..2000-05 dot-com-era window reads "
          f"{episode['frac_euphoric_in_window']:.1%} euphoric, z-stat rose from {episode['stat_at_window_start']:.4f} "
          f"to {episode['stat_at_window_end']:.4f} (max {episode['max_stat_in_window']:.4f}) in-window.")
    ...
    configs = csr.grid_configs()
    assert len(configs) <= 36
    assert csr.PRIMARY_CONFIG in configs
    ...
    primary_ci = configs.index(csr.PRIMARY_CONFIG)
    primary_cfg_id = cfg_id_for(primary_ci, csr.PRIMARY_CONFIG)
    primary_trial_id = f"new_038_{primary_cfg_id}"
    ...
    if check_4_1["pass"]:
        print("Sec 4.1 PASSED -- run scripts/v3/robustness_038_consumer_sentiment.py next "
    ...
    for asset in ASSETS:
        ...
        row = {"asset": asset}
        for fee in FEES:
            s, _ = run_one(daily, rf, csr.PRIMARY_CONFIG, fee)
            dca_s = dca_cache[asset][fee]
