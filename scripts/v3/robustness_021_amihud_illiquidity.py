"""Robustness diagnostics for family 021's primary configuration (plan sec
4.3). Modeled on scripts/v3/robustness_020_high52wk_tilt.py /
robustness_016_vix_contrarian.py: rolling windows (3y/5y for SP500/GOLD/
SILVER/OIL, 2y for BTC), block bootstrap raw+detrended, and placebo
circular-shift -- at the n_sims=60 time-budget convention, run ONLY IF sec
4.1 passes on the primary config (per this iteration's task and the
established precedent).

Placebo signal here is the primary config's own elevated-illiquidity
REGIME boolean array (per-day, from ami.compute_elevated_regime), NOT the
raw Amihud ratio itself -- circular-shifting the regime (which the weekly
decider reads) tests whether the specific temporal alignment of the
illiquidity-shock signal (not just "some correlated buy-more/buy-less
regime of the same overall frequency") is doing the work, mirroring
family 016's own placebo design (shifting its `elevated` array, not the
raw VIX level).
"""
...
def strat_builder(daily):
    return ami.make_amihud_illiquidity_decider(daily, WEEKLY_DEPOSIT, **ami.PRIMARY_CONFIG)
...
def main():
    ...
    # --- placebo: circular-shift the primary config's own elevated-regime array, SP500 ---
    cfg = ami.PRIMARY_CONFIG
    regime_cfg = {k: cfg[k] for k in ["illiq_lookback", "elevated_pct", "decay_days"]}
    _, regime_raw = ami.compute_elevated_regime(daily, **regime_cfg)
    ...
    def strategy_decide_from_signal(shifted_regime):
        return ami.make_decider_from_elevated_array(
            daily, WEEKLY_DEPOSIT, shifted_regime.astype(bool),
            buy_multiplier=cfg["buy_multiplier"], calm_fraction=cfg["calm_fraction"],
            max_buy_multiple=4.0,
        )

    df_pl, pct_w, pct_s = v3rob.placebo_circular_shift(
        daily, rf, WEEKLY_DEPOSIT, regime_raw.astype(float), strategy_decide_from_signal, bench_builder,
