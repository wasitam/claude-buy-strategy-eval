"""Run family 005 (time-series momentum sizing) end-to-end on development
data: implementation checks -> full grid -> trial logging -> DSR/N_eff ->
assess. Writes state/trials/new_005_*.csv, updates state/trial_counter.json,
and dumps results into families/005-tsmom-sizing/_run_output.json for the
results.md writer. Modeled directly on scripts/v3/run_003_vol_managed_sizing.py
(same single-asset, no-sell sizing-multiplier pattern).
"""
...
from src.backtest.v3.strategies import tsmom_sizing as tsm

ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
STATE_DIR = os.path.join(ROOT, "state")
TRIALS_DIR = os.path.join(STATE_DIR, "trials")
FAM_DIR = os.path.join(ROOT, "families", "005-tsmom-sizing")
FEES = [0.001, 0.0025]
WEEKLY_DEPOSIT = 500.0
ASSETS = ["SP500", "GOLD", "SILVER", "BTC", "OIL"]


def decider_builder_factory(daily, lookback_days, skip_days, mult_pos, mult_neg):
    def builder(enabled=True):
        return tsm.make_tsmom_decider(
            daily, WEEKLY_DEPOSIT, lookback_days=lookback_days, skip_days=skip_days,
            mult_pos=mult_pos, mult_neg=mult_neg, enabled=enabled,
        )
    return builder
...
def run_impl_checks(dev):
    ...
    rf = dev["rf"]
    builder = decider_builder_factory(daily, **tsm.PRIMARY_CONFIG)
    ...
    decide_tsm = builder(enabled=True)
    res_tsm = v3eng.run_single_asset(daily, WEEKLY_DEPOSIT, rf, decide_tsm).to_frame()
    out["no_negative_cash_units_tsmom"] = v3chk.check_no_negative_cash_or_units(res_tsm)
...
def run_one(daily, rf, cfg, fee):
    decide = tsm.make_tsmom_decider(daily, WEEKLY_DEPOSIT, **cfg)
...
def cfg_id_for(ci, cfg):
    return f"cfg{ci:02d}_lb{cfg['lookback_days']}_sk{cfg['skip_days']}_mp{cfg['mult_pos']}_mn{cfg['mult_neg']}"
...
def main():
    ...
    primary_ci = configs.index(tsm.PRIMARY_CONFIG)
    primary_cfg_id = cfg_id_for(primary_ci, tsm.PRIMARY_CONFIG)
    primary_trial_id = f"new_005_{primary_cfg_id}"
    ...
    for asset in ASSETS:
        ...
        row = {"asset": asset}
        for fee in FEES:
            s, _ = run_one(daily, rf, tsm.PRIMARY_CONFIG, fee)
            dca_s = dca_cache[asset][fee]
