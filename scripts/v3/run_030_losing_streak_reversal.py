"""Run family 030 (losing-streak / consecutive-down-days contrarian sizing)
...
assess. Writes state/trials/new_030_*.csv, updates
state/trial_counter.json, and dumps results into
families/030-losing-streak-reversal/_run_output.json for the results.md
writer. Modeled directly on scripts/v3/run_028_ma_crossover_regime.py and
scripts/v3/run_017_rsi2_reversion.py (same single-asset, no-sell,
sizing-tilt-with-cash-cap pattern; no known-episode check needed since this
signal has no "well-known historical episode" the way the MA-crossover
golden-cross does -- consecutive-down-day streaks are common and
mechanically defined, no external validation needed beyond the
non-degeneracy frequency check).
"""
...
from src.backtest.v3.strategies import losing_streak_reversal as lsr

ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
STATE_DIR = os.path.join(ROOT, "state")
TRIALS_DIR = os.path.join(STATE_DIR, "trials")
FAM_DIR = os.path.join(ROOT, "families", "030-losing-streak-reversal")
FEES = [0.001, 0.0025]
WEEKLY_DEPOSIT = 500.0
ASSETS = ["SP500", "GOLD", "SILVER", "BTC", "OIL"]

# Aggressive corner: lowest trigger threshold (fires most often), longest
# decay window (elevated state persists longest), highest multiplier and
# ceiling -- the config expected to spend cash fastest/hardest.
AGGRESSIVE_CFG = {
    "streak_threshold": 2, "buy_mult_streak": 2.0, "decay_days": 10,
    "max_lump_multiple": 3.0, "buy_mult_winning": lsr.BUY_MULT_WINNING,
}


def decider_builder_factory(daily, cfg):
    def builder(enabled=True):
        return lsr.make_losing_streak_decider(daily, WEEKLY_DEPOSIT, enabled=enabled, **cfg)
    return builder
...
def run_sanity_check(dev):
    """Pre-grid non-degeneracy check: primary config's elevated- and
    reduced-state frequency must be non-trivial (broad (1%, 60%) band) on
    all 5 core assets, confirming the streak-trigger signal fires
    non-trivially before any backtest result is trusted."""
    cfg = lsr.PRIMARY_CONFIG
    rows = []
    for asset in ASSETS:
        daily = dev["prices"][asset]
        state = lsr.compute_state(daily, cfg["streak_threshold"], cfg["decay_days"])
        n = len(state)
        elevated_frac = float(np.mean(state == "elevated"))
        reduced_frac = float(np.mean(state == "reduced"))
        down_streak, up_streak = lsr.compute_streaks(daily)
        n_down_triggers = int(np.sum(down_streak >= cfg["streak_threshold"]))
        n_up_triggers = int(np.sum(up_streak >= cfg["streak_threshold"]))
        non_degenerate = bool(
            0.01 < elevated_frac < 0.60 and 0.01 < reduced_frac < 0.60
            and n_down_triggers >= 5 and n_up_triggers >= 5
        )
        rows.append({
            "asset": asset, "n_days": int(n), "elevated_frac": elevated_frac,
            "reduced_frac": reduced_frac, "n_down_triggers": n_down_triggers,
            "n_up_triggers": n_up_triggers, "non_degenerate": non_degenerate,
        })
    return rows


def run_one(daily, rf, cfg, fee):
    decide = lsr.make_losing_streak_decider(daily, WEEKLY_DEPOSIT, **cfg)
...
def cfg_id_for(ci, cfg):
    return (f"cfg{ci:02d}_st{cfg['streak_threshold']}_bm{cfg['buy_mult_streak']}_"
            f"dd{cfg['decay_days']}_ml{cfg['max_lump_multiple']}")
...
def main():
    ...
    if hard_fail:
        raise SystemExit(f"Sanity check FAILED: {hard_fail} -- degenerate streak-trigger frequency.")

    configs = lsr.grid_configs()
    assert len(configs) <= 36
    assert lsr.PRIMARY_CONFIG in configs
    ...
    primary_ci = configs.index(lsr.PRIMARY_CONFIG)
    primary_cfg_id = cfg_id_for(primary_ci, lsr.PRIMARY_CONFIG)
    primary_trial_id = f"new_030_{primary_cfg_id}"
    ...
    if check_4_1["pass"]:
        print("Sec 4.1 PASSED -- run scripts/v3/robustness_030_losing_streak_reversal.py next "
    ...
    for asset in ASSETS:
        ...
        row = {"asset": asset}
        for fee in FEES:
            s, _ = run_one(daily, rf, lsr.PRIMARY_CONFIG, fee)
            dca_s = dca_cache[asset][fee]
