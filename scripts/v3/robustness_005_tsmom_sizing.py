"""Robustness diagnostics for family 005's primary configuration (plan sec
4.3). Modeled directly on scripts/v3/robustness_003_vol_managed_sizing.py
(same single-asset, continuous-sizing-multiplier pattern): rolling windows
(3y/5y for SP500/GOLD/SILVER/OIL, 2y for BTC), block bootstrap raw+detrended,
and placebo circular-shift -- at the same reduced n_sims=120 precedent set
by families 001-003 (state/bugfix_log.md judgment calls) since SP500's
...
function of the sign of trailing 12-month momentum, per prereg.md) --
circular-shifting m_t and rerunning tests whether the specific temporal
alignment of the momentum signal (not just "some correlated step-function
multiplier sequence") is doing the work, per the task's instruction to use
a circular-shift of the momentum signal for the placebo.
"""
...
def strat_builder(daily):
    return tsm.make_tsmom_decider(daily, WEEKLY_DEPOSIT, **tsm.PRIMARY_CONFIG)
...
def main():
    ...
    signal_raw = tsm.compute_multiplier(daily, **tsm.PRIMARY_CONFIG)
