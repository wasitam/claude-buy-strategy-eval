"""Run family 007 (day-of-week deposit timing) end-to-end on development
data: implementation checks -> full grid -> trial logging -> DSR/N_eff ->
assess. Writes state/trials/new_007_*.csv, updates state/trial_counter.json,
and dumps results into families/007-day-of-week/_run_output.json for the
results.md writer. Modeled directly on scripts/v3/run_006_turn_of_month.py.
"""
...
from src.backtest.v3.strategies import day_of_week as dow

ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
STATE_DIR = os.path.join(ROOT, "state")
TRIALS_DIR = os.path.join(STATE_DIR, "trials")
FAM_DIR = os.path.join(ROOT, "families", "007-day-of-week")
FEES = [0.001, 0.0025]
...
def decider_builder_factory(daily, cfg):
    resolved = dow._resolve(cfg)

    def builder(enabled=True):
        return dow.make_dow_decider(daily, WEEKLY_DEPOSIT, enabled=enabled, **resolved)
    return builder
...
def run_impl_checks(dev):
    ...
    rf = dev["rf"]
    builder = decider_builder_factory(daily, dow.PRIMARY_CONFIG)
    ...
    decide_dow = builder(enabled=True)
    res_dow = v3eng.run_single_asset(daily, WEEKLY_DEPOSIT, rf, decide_dow).to_frame()
    out["no_negative_cash_units_dow"] = v3chk.check_no_negative_cash_or_units(res_dow)
    ...
    # Also check the Sunday-target arm's no-lookahead & non-negativity on
    # BTC, since that arm has the most unusual behavior (Sunday IS a
    # trading day for BTC, unlike the 4 non-BTC assets) and is worth an
    # extra check beyond the primary config alone.
    btc_daily = dev["prices"]["BTC"]
    sunday_cfg = {**dow.PRIMARY_CONFIG, "target_weekday": "Sunday"}
    sunday_builder = decider_builder_factory(btc_daily, sunday_cfg)
    decide_sun = sunday_builder(enabled=True)
    res_sun = v3eng.run_single_asset(btc_daily, WEEKLY_DEPOSIT, rf, decide_sun).to_frame()
    out["no_negative_cash_units_dow_sunday_btc"] = v3chk.check_no_negative_cash_or_units(res_sun)
    out["no_lookahead_sunday_btc"] = v3chk.check_no_lookahead(btc_daily, rf, sunday_builder, t_check=1200)
    ...
    # cash earning IRX the whole time. Mirrors run_004/run_006's
    # capital-neutrality check exactly. This family never sells, so net
    # deployed == gross bought.
    cash_only = np.zeros(len(res_dow))
    rf_arr = rf.reindex(res_dow.index).fillna(0.0).to_numpy()
    dep_arr = res_dow["deposit"].to_numpy()
    c = 0.0
    for i in range(len(res_dow)):
        c *= 1.0 + rf_arr[i]
        c += dep_arr[i]
        cash_only[i] = c
    net_deployed = (res_dow["buy_usd"] - res_dow["sell_usd"]).cumsum().to_numpy()
...
def run_one(daily, rf, cfg, fee):
    resolved = dow._resolve(cfg)
    decide = dow.make_dow_decider(daily, WEEKLY_DEPOSIT, **resolved)
...
def cfg_id_for(ci, cfg):
    return (f"cfg{ci:02d}_tw{cfg['target_weekday']}_mt{cfg['mild_tilt_fraction']}"
...
def main():
    ...
    primary_ci = configs.index(dow.PRIMARY_CONFIG)
    primary_cfg_id = cfg_id_for(primary_ci, dow.PRIMARY_CONFIG)
    primary_trial_id = f"new_007_{primary_cfg_id}"
    ...
    # CSCV PBO diagnostic. Pooled per-asset weekly strategy returns across
    # the grid -- both BTC (the literature-anchor asset for this family)
    # and SP500 (kept for cross-family comparability with 006), following
    # each family's own most-relevant asset for the diagnostic.
    cscv_btc = v3dsr.cscv_pbo({cfg_id: per_config_asset_weekly[(cfg_id, "BTC", 0.001)] for cfg_id in grid_df["config_id"].unique()}, n_splits=8)
    ...
    print("CSCV PBO (BTC grid):", cscv_btc)
    ...
    for asset in ASSETS:
        ...
        row = {"asset": asset}
        for fee in FEES:
            s, _ = run_one(daily, rf, dow.PRIMARY_CONFIG, fee)
            dca_s = dca_cache[asset][fee]
