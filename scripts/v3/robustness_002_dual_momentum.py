"""Sec 4.3 robustness checks for family 002 (dual momentum rotation),
primary config, portfolio-level. Run counts scoped down from the plan's 500
per prereg.md's cost-scoping judgment call (60 sims for bootstrap/placebo,
half of family 001's already-reduced 120, since each sim reruns the full
5-asset portfolio engine).
"""
...
def main():
    ...
    def s_builder(aw, iw):
        rf = daily_rf.reindex(iw).ffill().fillna(0.0)
        c = {a: aw[a]["Close"].to_numpy() for a in ASSETS}
        we = week_end_flags(iw)
        return dm.make_dual_momentum_decider(c, rf, iw, we, WEEKLY_DEPOSIT_PER_ASSET, ASSETS, **dm.PRIMARY_CONFIG, enabled=True)

    def b_builder(aw, iw):
        return v3pe.make_fixed_weight_dca_decider(WEEKLY_DEPOSIT_PER_ASSET, ASSETS)

    # --- sec 4.3 rolling windows: 3-year (only window that fits meaningfully
    # given BTC-gated portfolio history is ~5.3 years total in dev) ---
    ...
    real_decide = dm.make_dual_momentum_decider(closes, daily_rf, idx, is_week_end, WEEKLY_DEPOSIT_PER_ASSET,
                                                  ASSETS, **dm.PRIMARY_CONFIG, enabled=True)
    ...
    weights_full = dm.compute_target_weights(closes, daily_rf, idx, **dm.PRIMARY_CONFIG)
    weekend_weights = weights_full.loc[is_week_end]

    def strategy_decide_from_weekend_weights(shifted):
        return dm.make_decider_from_weekend_weights(shifted, is_week_end, idx, closes, ASSETS)
