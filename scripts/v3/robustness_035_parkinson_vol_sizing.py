"""Robustness diagnostics for family 035's primary configuration (plan sec
4.3). Modeled directly on scripts/v3/robustness_034_low52wk_tilt.py (same
...
prereg.md, run only because sec 4.1 passed.

Placebo signal here is the strategy's own sizing multiplier m_t (a
continuous function of the Parkinson range-vol ratio, per prereg.md) --
circular-shifting m_t and rerunning tests whether the specific temporal
alignment of the range-based volatility signal (not just "some correlated
...
def strat_builder(daily):
    return pvs.make_parkinson_vol_decider(daily, WEEKLY_DEPOSIT, **pvs.PRIMARY_CONFIG)
...
def main():
    ...
    signal_raw = pvs.compute_multiplier(daily, pvs.PRIMARY_CONFIG["park_lookback_days"], pvs.PRIMARY_CONFIG["ref_lookback_days"],
                                         pvs.PRIMARY_CONFIG["min_mult"], pvs.PRIMARY_CONFIG["max_mult"])
