"""Run family 019 (OECD CLI regime switch) end-to-end on development data:
implementation checks -> pre-grid non-degeneracy sanity check -> full grid
-> trial logging -> DSR/N_eff -> assess. Writes state/trials/new_019_*.csv,
updates state/trial_counter.json, and dumps results into
families/019-oecd-cli-regime/_run_output.json for the results.md writer.
Modeled directly on scripts/v3/run_011_credit_stress_filter.py (same
single-asset, no-sell, banking-mechanic, weekly-decision-cadence, macro
point-in-time signal pattern).
"""
...
from src.backtest.v3.strategies import oecd_cli_regime as cli

ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
STATE_DIR = os.path.join(ROOT, "state")
TRIALS_DIR = os.path.join(STATE_DIR, "trials")
FAM_DIR = os.path.join(ROOT, "families", "019-oecd-cli-regime")
FEES = [0.001, 0.0025]
...
def decider_builder_factory(daily, cfg):
    def builder(enabled=True):
        return cli.make_cli_regime_decider(daily, WEEKLY_DEPOSIT, enabled=enabled, **cfg)
    return builder


def run_sanity_check(dev):
    """Pre-grid non-degeneracy sanity check (task instruction, families
    015/016/018 convention): confirm the primary config's weak condition
    fires non-trivially (not near-0%, not near-100% of dev days) on every
    core asset, BEFORE the full grid is trusted."""
    cfg = cli.PRIMARY_CONFIG
    rows = []
    for asset in ASSETS:
        daily = dev["prices"][asset]
        weak = cli.compute_weak(daily, cfg["normalization_method"], cfg["lookback_years"], cfg["threshold_level"])
...
def run_impl_checks(dev):
    ...
    # Capital deployed never exceeds cumulative deposits + interest: cash +
    # market value of units bought should never exceed cumulative deposits
    # credited so far plus interest earned on banked cash (i.e. total
    # capital in the system is conserved -- nothing manufactured).
    cum_deposit_plus_interest = res_primary["cash"] + res_primary["units"] * daily["Close"].reindex(res_primary.index).ffill()
    # This is just NAV, which by construction of the engine (cash-capped
    # buys, no leverage) cannot exceed cumulative deposits + interest +
    # price appreciation -- the real invariant we check is that cash never
    # went negative (already checked) and units bought never exceed what
    # cash at that point could afford, which check_no_negative_cash_or_units
    # already enforces via the engine's cash-capped fill logic. Recorded
    # explicitly here per the task's checklist wording.
    ...
    # documented 60-day lag -- proving the lag actually matters (not a
    # no-op) -- while every real backtest below uses the documented lag
    # only. Especially important here given USALOLITONOSTSAM is monthly,
    # revisable, and publication-lagged, per the task brief.
    weak_documented = cli.compute_weak(daily, cfg["normalization_method"], cfg["lookback_years"], cfg["threshold_level"])
    weak_zero_lag = cli.compute_weak(daily, cfg["normalization_method"], cfg["lookback_years"], cfg["threshold_level"], lag_override_days=0)
    frac_diff = float((weak_documented != weak_zero_lag).mean())
    out["lag_matters_zero_vs_documented_frac_days_differ"] = frac_diff
    out["point_in_time_macro"] = bool(frac_diff > 0.0)  # lag has a real, nonzero effect

    return out


def run_one(daily, rf, cfg, fee):
    decide = cli.make_cli_regime_decider(daily, WEEKLY_DEPOSIT, **cfg)
...
def cfg_id_for(ci, cfg):
    return (f"cfg{ci:02d}_{cfg['normalization_method']}_lb{cfg['lookback_years']}"
            f"_tl{cfg['threshold_level']}_tf{cfg['weak_tilt_fraction']}")
...
def main():
    ...
    primary_ci = configs.index(cli.PRIMARY_CONFIG)
    primary_cfg_id = cfg_id_for(primary_ci, cli.PRIMARY_CONFIG)
    primary_trial_id = f"new_019_{primary_cfg_id}"
    ...
    for asset in ASSETS:
        ...
        row = {"asset": asset}
        for fee in FEES:
            s, _ = run_one(daily, rf, cli.PRIMARY_CONFIG, fee)
            dca_s = dca_cache[asset][fee]
