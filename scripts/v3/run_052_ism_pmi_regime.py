"""Run family 052 (ISM Manufacturing PMI regime deposit sizing) end-to-end
on development data: implementation checks -> pre-grid sanity checks ->
full grid -> trial logging -> DSR/N_eff -> assess. Writes
state/trials/new_052_*.csv, updates state/trial_counter.json, and dumps
results into families/052-ism-pmi-regime/_run_output.json for the
results.md writer. Modeled directly on
scripts/v3/run_050_ted_funding_stress.py (same single-asset, no-sell,
banking-mechanic, weekly-decision-cadence, macro point-in-time signal
pattern -- monthly instead of daily observation cadence).
"""
...
FAM_DIR = os.path.join(ROOT, "families", "052-ism-pmi-regime")
FEES = [0.001, 0.0025]
...
def decider_builder_factory(daily, cfg):
    def builder(enabled=True):
        return ism.make_ism_pmi_decider(daily, WEEKLY_DEPOSIT, enabled=enabled, **cfg)
    return builder


def run_sanity_checks(dev):
    """Pre-grid sanity checks (prereg.md): (1) non-degeneracy -- the
    primary config's weak condition fires non-trivially on every core
    asset; (2) known-episode check -- the 2008-09 post-Lehman collapse
    must read mostly weak, strictly within dev dates."""
    cfg = ism.PRIMARY_CONFIG
    rows = []
    for asset in ASSETS:
        daily = dev["prices"][asset]
        weak = ism.compute_weak(daily, cfg["lookback_years"], cfg["weak_pctile"], cfg["persistence_months"])
        frac_weak = float(weak.mean())
        rows.append({"asset": asset, "dev_days": len(weak), "weak_days": int(weak.sum()), "frac_weak": frac_weak})

    # Known-episode check: Sep 2008 - Mar 2009 post-Lehman manufacturing
    # collapse, checked on the signal itself (asset-agnostic), well
    # within dev dates (pre-2020).
    weak_at_signal_dates = ism._weak_at_signal_dates(
        cfg["lookback_years"], cfg["weak_pctile"], cfg["persistence_months"]
    )
    raw = ism.fetch_raw_signal()
    window = weak_at_signal_dates.loc["2008-09-01":"2009-03-31"]
    raw_window = raw.loc["2008-09-01":"2009-03-31"]
    episode = {
        "window": "2008-09-01..2009-03-31",
        "n_obs": int(len(window)),
        "frac_weak_in_window": float(window.mean()) if len(window) else float("nan"),
        "pmi_at_window_start": float(raw_window.iloc[0]) if len(raw_window) else float("nan"),
        "pmi_at_window_end": float(raw_window.iloc[-1]) if len(raw_window) else float("nan"),
        "min_pmi_in_window": float(raw_window.min()) if len(raw_window) else float("nan"),
    }
    return rows, episode
...
def run_impl_checks(dev):
    ...
    # only.
    weak_documented = ism.compute_weak(daily, cfg["lookback_years"], cfg["weak_pctile"], cfg["persistence_months"])
    weak_zero_lag = ism.compute_weak(daily, cfg["lookback_years"], cfg["weak_pctile"], cfg["persistence_months"], lag_override_days=0)
    frac_diff = float((weak_documented != weak_zero_lag).mean())
...
def run_one(daily, rf, cfg, fee):
    decide = ism.make_ism_pmi_decider(daily, WEEKLY_DEPOSIT, **cfg)
...
def cfg_id_for(ci, cfg):
    return (f"cfg{ci:02d}_lb{cfg['lookback_years']}_wp{cfg['weak_pctile']}"
            f"_tf{cfg['weak_tilt_fraction']}_pm{cfg['persistence_months']}")
...
def main():
    ...
    for row in sanity:
        if not (0.02 <= row["frac_weak"] <= 0.98):
            raise SystemExit(f"Sanity check FAILED for {row['asset']}: frac_weak={row['frac_weak']:.4f} "
                              "outside (2%, 98%) -- degenerate regime, stopping before grid.")
    if episode["n_obs"] == 0:
        raise SystemExit("Known-episode check FAILED: no Sep2008-Mar2009 PMI observations found in dev data.")
    if episode["frac_weak_in_window"] < 0.5:
        raise SystemExit(
            f"Known-episode check FAILED: the primary config reads only "
            f"{episode['frac_weak_in_window']:.1%} of the Sep2008-Mar2009 post-Lehman collapse window as WEAK; "
            "expected a clear majority -- signal construction does not match the well-known episode."
        )
    print(f"Known-episode check PASSED: Sep2008-Mar2009 post-Lehman window reads "
          f"{episode['frac_weak_in_window']:.1%} weak, reconstructed PMI fell from "
          f"{episode['pmi_at_window_start']:.3f} to {episode['pmi_at_window_end']:.3f} "
          f"(min {episode['min_pmi_in_window']:.3f}) in-window.")
    ...
    configs = ism.grid_configs()
    assert len(configs) <= 36
    assert ism.PRIMARY_CONFIG in configs
    ...
    primary_ci = configs.index(ism.PRIMARY_CONFIG)
    primary_cfg_id = cfg_id_for(primary_ci, ism.PRIMARY_CONFIG)
    primary_trial_id = f"new_052_{primary_cfg_id}"
    ...
    for asset in ASSETS:
        ...
        row = {"asset": asset}
        for fee in FEES:
            s, _ = run_one(daily, rf, ism.PRIMARY_CONFIG, fee)
            dca_s = dca_cache[asset][fee]
