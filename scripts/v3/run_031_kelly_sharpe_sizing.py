"""Run family 031 (Kelly-fraction-style trailing-Sharpe sizing) end-to-end
on development data: implementation checks -> pre-grid non-degeneracy
sanity check (+ concrete numeric contrast vs families 003/005) -> full
grid -> trial logging -> DSR/N_eff -> assess. Writes
state/trials/new_031_*.csv, updates state/trial_counter.json, and dumps
results into families/031-kelly-sharpe-sizing/_run_output.json for the
results.md writer. Modeled directly on
scripts/v3/run_030_losing_streak_reversal.py (same single-asset, no-sell,
sizing-tilt-with-cash-cap pattern).
"""
...
from src.backtest.v3.strategies import kelly_sharpe_sizing as ksz

ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
STATE_DIR = os.path.join(ROOT, "state")
TRIALS_DIR = os.path.join(STATE_DIR, "trials")
FAM_DIR = os.path.join(ROOT, "families", "031-kelly-sharpe-sizing")
FEES = [0.001, 0.0025]
WEEKLY_DEPOSIT = 500.0
ASSETS = ["SP500", "GOLD", "SILVER", "BTC", "OIL"]

# Aggressive corner: shortest, most reactive Sharpe window, highest
# sensitivity, lowest floor -- the config expected to swing the
# multiplier hardest and most often.
AGGRESSIVE_CFG = {"sharpe_window": 40, "pctile_lookback": 504, "k": 1.5, "min_mult": 0.25}


def decider_builder_factory(daily, cfg):
    def builder(enabled=True):
        return ksz.make_kelly_sharpe_decider(daily, WEEKLY_DEPOSIT, enabled=enabled, **cfg)
    return builder
...
def run_impl_checks(dev):
    ...
    agg_builder = decider_builder_factory(daily, AGGRESSIVE_CFG)
...
def run_sanity_check(dev):
    """Pre-grid non-degeneracy check: primary config's elevated (m_t>1)
    and depressed (m_t<1) day-fractions must both fall in a broad
    (20%, 80%) band on all 5 core assets, confirming the percentile-rank
    signal fires non-trivially (close to the ~50/50 split expected by
    construction) before any grid result is trusted."""
    cfg = ksz.PRIMARY_CONFIG
    rows = []
    for asset in ASSETS:
        daily = dev["prices"][asset]
        m = ksz.compute_multiplier(daily, **cfg)
        n = len(m)
        n_have_history = int(np.sum(~np.isnan(ksz.compute_sharpe_signal(daily, cfg["sharpe_window"]))))
        elevated_frac = float(np.mean(m > 1.0))
        depressed_frac = float(np.mean(m < 1.0))
        non_degenerate = bool(
            0.20 < elevated_frac < 0.80 and 0.20 < depressed_frac < 0.80
            and n_have_history >= cfg["pctile_lookback"]
        )
        rows.append({
            "asset": asset, "n_days": int(n), "n_have_sharpe_history": n_have_history,
            "elevated_frac": elevated_frac, "depressed_frac": depressed_frac,
            "non_degenerate": non_degenerate,
        })
    return rows


def numeric_contrast(dev):
    """Concrete numeric contrast (real dev data): find (asset, date) pairs
    across DIFFERENT assets with near-identical trailing Sharpe (primary
    config's sharpe_window) but very different trailing return/vol
    magnitudes -- proves the signal isn't secretly redundant with family
    003 (vol alone) or family 005 (sign of return alone). Documented in
    prereg.md with pre-computed values; recomputed here at run time for
    the record."""
    window = ksz.PRIMARY_CONFIG["sharpe_window"]
    stats = {}
    for asset in ASSETS:
        daily = dev["prices"][asset]
        ...
        log_ret[0] = 0.0
        s = pd.Series(log_ret, index=daily.index)
        mean = s.rolling(window, min_periods=window).mean() * 252
        vol = s.rolling(window, min_periods=window).std(ddof=1) * np.sqrt(252)
        sharpe = mean / vol
        stats[asset] = pd.DataFrame({"mean": mean, "vol": vol, "sharpe": sharpe}).dropna()

    pairs = [
        ("BTC", pd.Timestamp("2018-01-22"), "SP500", pd.Timestamp("1993-12-20")),
        ("BTC", pd.Timestamp("2018-02-03"), "GOLD", pd.Timestamp("2017-11-28")),
    ]
    out = []
    for a1, d1, a2, d2 in pairs:
        try:
            ...
            continue
        out.append({
            "asset_1": a1, "date_1": str(d1.date()), "mean_1": float(r1["mean"]), "vol_1": float(r1["vol"]), "sharpe_1": float(r1["sharpe"]),
            "asset_2": a2, "date_2": str(d2.date()), "mean_2": float(r2["mean"]), "vol_2": float(r2["vol"]), "sharpe_2": float(r2["sharpe"]),
            "vol_ratio": float(r1["vol"] / r2["vol"]),
        })
    return out


def run_one(daily, rf, cfg, fee):
    decide = ksz.make_kelly_sharpe_decider(daily, WEEKLY_DEPOSIT, **cfg)
...
def cfg_id_for(ci, cfg):
    return (f"cfg{ci:02d}_sw{cfg['sharpe_window']}_pl{cfg['pctile_lookback']}_"
            f"k{cfg['k']}_mm{cfg['min_mult']}")
...
def main():
    ...
    hard_fail = [row for row in sanity if not row["non_degenerate"]]
    if hard_fail:
        raise SystemExit(f"Sanity check FAILED: {hard_fail} -- degenerate percentile-rank frequency.")

    print("Computing concrete numeric contrast (real data)...")
    contrast = numeric_contrast(dev)
    print(json.dumps(contrast, indent=2))

    configs = ksz.grid_configs()
    assert len(configs) <= 36
    assert ksz.PRIMARY_CONFIG in configs
    ...
    primary_ci = configs.index(ksz.PRIMARY_CONFIG)
    primary_cfg_id = cfg_id_for(primary_ci, ksz.PRIMARY_CONFIG)
    primary_trial_id = f"new_031_{primary_cfg_id}"
    ...
    if check_4_1["pass"]:
        print("Sec 4.1 PASSED -- run scripts/v3/robustness_031_kelly_sharpe_sizing.py next "
    ...
    for asset in ASSETS:
        ...
        row = {"asset": asset}
        for fee in FEES:
            s, _ = run_one(daily, rf, ksz.PRIMARY_CONFIG, fee)
            dca_s = dca_cache[asset][fee]
