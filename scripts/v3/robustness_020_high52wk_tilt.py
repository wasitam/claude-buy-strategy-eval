"""Robustness diagnostics for family 020's primary configuration (plan sec
...
prereg.md and this iteration's task instructions (sec 4.1 already passed
narrowly at exactly 3/5 core assets, so sec 4.3 is run per the task's
explicit "and sec 4.3 in full if sec 4.1 passes" instruction, even though
sec 4.2/4.4 are already known to fail decisively -- same "moot for the
verdict, run anyway per the established precedent" pattern family 005 used).

Placebo signal here is the strategy's own sizing multiplier m_t (a step
function of prox_t = close_t/high52_t, per prereg.md) -- circular-shifting
m_t and rerunning tests whether the specific temporal alignment of the
52-week-high proximity signal (not just "some correlated step-function
...
def strat_builder(daily):
    return h52.make_high52wk_tilt_decider(daily, WEEKLY_DEPOSIT, **h52.PRIMARY_CONFIG)
...
def main():
    ...
    signal_raw = h52.compute_multiplier(daily, h52.PRIMARY_CONFIG["window_def"], h52.PRIMARY_CONFIG["ladder"],
                                         h52.PRIMARY_CONFIG["mult_near"], h52.PRIMARY_CONFIG["max_lump_cap"])
