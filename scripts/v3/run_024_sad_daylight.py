"""Run family 024 (SAD daylight-length deposit timing) end-to-end on
...
state/trials/new_024_*.csv, updates state/trial_counter.json, and dumps
results into families/024-sad-daylight/_run_output.json for the results.md
writer. Modeled on scripts/v3/run_023_realized_skewness.py and
run_018_halloween_seasonal.py (calendar-signal, banking-reserve pattern).
"""
...
from src.backtest.v3.strategies import sad_daylight as sad

ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
STATE_DIR = os.path.join(ROOT, "state")
TRIALS_DIR = os.path.join(STATE_DIR, "trials")
FAM_DIR = os.path.join(ROOT, "families", "024-sad-daylight")
FEES = [0.001, 0.0025]
WEEKLY_DEPOSIT = 500.0
ASSETS = ["SP500", "GOLD", "SILVER", "BTC", "OIL"]

# Most-aggressive corner per prereg.md's implementation-check plan: steepest
# response + widest multiplier swing + most aggressive burst cap.
AGGRESSIVE_CORNER = {"tilt_strength": 0.7, "power": 2.0, "max_lump_multiple": 12}


def decider_builder_factory(daily, cfg):
    def builder(enabled=True):
        return sad.make_sad_daylight_decider(daily, WEEKLY_DEPOSIT, enabled=enabled, **cfg)
    return builder


def run_astronomical_sanity_check():
    """Spot-check D(t) against known solstice/equinox values across several
    separate years (leap and non-leap), before any backtest is trusted."""
    checks = []
    for year in [2001, 2004, 2016, 2019]:  # mix of leap (2004, 2016) and non-leap years
        dec21 = sad.daylight_hours(pd.to_datetime([f"{year}-12-21"]))[0]
        jun21 = sad.daylight_hours(pd.to_datetime([f"{year}-06-21"]))[0]
        mar_eq = sad.daylight_hours(pd.to_datetime([f"{year}-03-20"]))[0]
        sep_eq = sad.daylight_hours(pd.to_datetime([f"{year}-09-23"]))[0]
        row = {
            "year": year, "dec21_hours": float(dec21), "jun21_hours": float(jun21),
            "mar_equinox_hours": float(mar_eq), "sep_equinox_hours": float(sep_eq),
            "dec21_is_shortest": bool(dec21 < mar_eq and dec21 < sep_eq and dec21 < jun21),
            "jun21_is_longest": bool(jun21 > mar_eq and jun21 > sep_eq and jun21 > dec21),
            "equinoxes_near_12h": bool(abs(mar_eq - 12.0) < 0.3 and abs(sep_eq - 12.0) < 0.3),
            "dec21_near_known_9h20m": bool(abs(dec21 - 9.33) < 0.3),
            "jun21_near_known_14h49m": bool(abs(jun21 - 14.82) < 0.3),
        }
        checks.append(row)
    return checks
...
def run_sanity_check(dev):
    """Pre-grid non-degeneracy sanity check (prereg.md): mult(t) must be
    genuinely continuous and varying (not constant, not pinned at its
    bounds) across a full year of trading days, on every core asset."""
    cfg = {k: sad.PRIMARY_CONFIG[k] for k in ["tilt_strength", "power"]}
    rows = []
    for asset in ASSETS:
        daily = dev["prices"][asset]
        mult = sad.compute_sad_multiplier(daily, **cfg)
        n_distinct = int(np.unique(np.round(mult, 6)).size)
        rows.append({
            "asset": asset, "n_days": int(len(mult)),
            "mult_min": float(mult.min()), "mult_max": float(mult.max()),
            "mult_std": float(mult.std()), "n_distinct_values": n_distinct,
            "non_degenerate": bool(mult.std() > 0.05 and n_distinct > 50
                                    and mult.min() < 0.95 and mult.max() > 1.05),
        })
    return rows
...
def run_impl_checks(dev):
    ...
    # minus cumulative deposits must never exceed that ceiling.
    ...
    out["point_in_time_macro"] = True  # calendar-only signal, no macro dependency
    return out


def run_one(daily, rf, cfg, fee):
    decide = sad.make_sad_daylight_decider(daily, WEEKLY_DEPOSIT, **cfg)
...
def cfg_id_for(ci, cfg):
    return f"cfg{ci:02d}_ts{cfg['tilt_strength']}_p{cfg['power']}_ml{cfg['max_lump_multiple']}"
...
def main():
    ...
    print("Running astronomical sanity spot-check...")
    astro = run_astronomical_sanity_check()
    print(json.dumps(astro, indent=2))
    for row in astro:
        if not (row["dec21_is_shortest"] and row["jun21_is_longest"] and row["equinoxes_near_12h"]
                and row["dec21_near_known_9h20m"] and row["jun21_near_known_14h49m"]):
            raise SystemExit(f"Astronomical sanity check FAILED for year {row['year']}: {row}")
    ...
    for row in sanity:
        if not row["non_degenerate"]:
            raise SystemExit(f"Sanity check FAILED for {row['asset']}: {row} -- degenerate multiplier, "
                              "stopping before grid.")

    configs = sad.grid_configs()
    assert len(configs) <= 36
    assert sad.PRIMARY_CONFIG in configs
    ...
    primary_ci = configs.index(sad.PRIMARY_CONFIG)
    primary_cfg_id = cfg_id_for(primary_ci, sad.PRIMARY_CONFIG)
    primary_trial_id = f"new_024_{primary_cfg_id}"
    ...
    # --- sec 4.3, only if sec 4.1 passes (established loop precedent). Run
    # as a separate scripts/v3/robustness_024_sad_daylight.py, modeled on
    # robustness_020_high52wk_tilt.py, so this script's signature stays
    # simple; sec 4.1's own result decides whether that script gets run.
    if check_4_1["pass"]:
        print("Sec 4.1 PASSED -- run scripts/v3/robustness_024_sad_daylight.py next "
        ...
        print("Sec 4.1 FAILED -- skipping sec 4.3 per established loop precedent "
              "(006/007/008/011/013/014/015/016/017/018/019/021/022/023).")
    ...
    for asset in ASSETS:
        ...
        row = {"asset": asset}
        for fee in FEES:
            s, _ = run_one(daily, rf, sad.PRIMARY_CONFIG, fee)
            dca_s = dca_cache[asset][fee]
