"""Robustness diagnostics for family 046's primary configuration (plan sec
...
array m_t (derived from the percentile-ranked, inverted VVIX vol-of-vol
signal), NOT the raw price series -- circular-shifting m_t tests whether
the specific temporal alignment of the VVIX signal (not just "some
correlated sizing pattern of the same overall distribution") is doing
the work.
"""
...
def strat_builder(daily):
    return vvs.make_vvix_regime_decider(daily, WEEKLY_DEPOSIT, **vvs.PRIMARY_CONFIG)
...
def main():
    ...
    m_real = vvs.compute_multiplier(daily, cfg['vvix_lookback'], cfg['k'], cfg['min_mult'], cfg['max_mult'])
    ...
    def strategy_decide_from_signal(shifted_m):
        lump_cap = vvs.MAX_LUMP_MULTIPLE * WEEKLY_DEPOSIT
