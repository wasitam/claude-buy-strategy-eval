"""Run family 021 (Amihud illiquidity-shock sizing) end-to-end on
...
state/trials/new_021_*.csv, updates state/trial_counter.json, and dumps
results into families/021-amihud-illiquidity/_run_output.json for the
results.md writer. Modeled on scripts/v3/run_016_vix_contrarian.py (same
weekly-decision, elevated-regime, calm-fraction-banking pattern).
"""
...
from src.backtest.v3.strategies import amihud_illiquidity as ami

ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
STATE_DIR = os.path.join(ROOT, "state")
TRIALS_DIR = os.path.join(STATE_DIR, "trials")
FAM_DIR = os.path.join(ROOT, "families", "021-amihud-illiquidity")
FEES = [0.001, 0.0025]
WEEKLY_DEPOSIT = 500.0
ASSETS = ["SP500", "GOLD", "SILVER", "BTC", "OIL"]

# Most-aggressive corner per prereg.md's implementation-check plan: lowest
# threshold (most days flagged elevated), highest multiplier, most
# aggressive banking, longest decay.
AGGRESSIVE_CORNER = {
    "illiq_lookback": 126, "elevated_pct": 90.0, "decay_days": 10,
    "buy_multiplier": 2.0, "calm_fraction": 0.85,
}


def decider_builder_factory(daily, cfg):
    def builder(enabled=True):
        return ami.make_amihud_illiquidity_decider(daily, WEEKLY_DEPOSIT, enabled=enabled, **cfg)
    return builder


def run_sanity_check(dev):
    """Pre-grid non-degeneracy sanity check (prereg.md): primary config's
    elevated-illiquidity trigger must fire on strictly between 2% and 60%
    of week-end decision days, on every core asset -- guards against a
    futures-roll artifact (GOLD/SILVER/OIL) producing a degenerate
    near-0%/near-100% trigger rate."""
    cfg = {k: ami.PRIMARY_CONFIG[k] for k in ["illiq_lookback", "elevated_pct", "decay_days"]}
    rows = []
    for asset in ASSETS:
        daily = dev["prices"][asset]
        is_week_end = v3eng.week_end_flags(daily.index)
        _, regime = ami.compute_elevated_regime(daily, **cfg)
        weekend_regime = regime[is_week_end]
        frac = float(weekend_regime.mean())
        rows.append({
            "asset": asset, "n_weekend_days": int(is_week_end.sum()),
            "frac_weekend_elevated": frac,
            "non_degenerate": bool(0.02 <= frac <= 0.60),
        })
    return rows
...
def run_impl_checks(dev):
    ...
    # sec 3.2 bullet 2): over SP500's ~90-year development history, cash
    # sitting idle at historical IRX rates compounds to a LARGE cumulative
    # interest figure (this family banks a real, sometimes multi-decade,
    # cash reserve between elevated-regime episodes) -- a flat 5%-of-deposits
    # tolerance (the convention borrowed from shorter-reserve-duration prior
    # families) is too strict here and flags real compounded interest income
    # as a false positive. The correct, principled bound: rerun the SAME
    # daily_rf series with a "never invest" decider (100% of every deposit
    # sits in cash, compounding at the historical IRX path every day) --
    # this gives the maximum interest any cash trajectory bounded by the
    # engine's own cash-cap invariant could possibly have earned. buy_usd is
    # unconditionally clipped to cash at fill time by the engine itself
    # (engine.py: `buy_usd = max(0.0, min(buy_usd, cash))`), so capital
    # deployed can never exceed deposits + interest under ANY decider --
    # this check re-verifies that invariant empirically rather than assuming
    # it, using the never-invest run as the interest ceiling.
    ...
    out["point_in_time_macro"] = True  # price-and-volume-only signal, no macro dependency
    return out


def run_one(daily, rf, cfg, fee):
    decide = ami.make_amihud_illiquidity_decider(daily, WEEKLY_DEPOSIT, **cfg)
...
def cfg_id_for(ci, cfg):
    return (f"cfg{ci:02d}_lb{cfg['illiq_lookback']}_ep{cfg['elevated_pct']:.0f}"
            f"_dd{cfg['decay_days']}_bm{cfg['buy_multiplier']}_cf{cfg['calm_fraction']}")
...
def main():
    ...
    for row in sanity:
        if not row["non_degenerate"]:
            raise SystemExit(f"Sanity check FAILED for {row['asset']}: "
                              f"frac_weekend_elevated={row['frac_weekend_elevated']:.4f} "
                              "outside (2%, 60%) -- degenerate trigger, stopping before grid.")

    configs = ami.grid_configs()
    ...
    primary_ci = configs.index(ami.PRIMARY_CONFIG)
    primary_cfg_id = cfg_id_for(primary_ci, ami.PRIMARY_CONFIG)
    primary_trial_id = f"new_021_{primary_cfg_id}"
    ...
    for asset in ASSETS:
        ...
        row = {"asset": asset}
        for fee in FEES:
            s, _ = run_one(daily, rf, ami.PRIMARY_CONFIG, fee)
            dca_s = dca_cache[asset][fee]
