"""Run family 050 (money-market funding-stress / TED-spread regime
sizing) end-to-end on development data: implementation checks -> pre-grid
sanity checks -> full grid -> trial logging -> DSR/N_eff -> assess.
Writes state/trials/new_050_*.csv, updates state/trial_counter.json, and
dumps results into families/050-ted-funding-stress/_run_output.json for
the results.md writer. Modeled directly on
scripts/v3/run_032_m2_growth_regime.py (same single-asset, no-sell,
...
from src.backtest.v3.strategies import ted_funding_stress as ted

ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
STATE_DIR = os.path.join(ROOT, "state")
TRIALS_DIR = os.path.join(STATE_DIR, "trials")
FAM_DIR = os.path.join(ROOT, "families", "050-ted-funding-stress")
FEES = [0.001, 0.0025]
...
def decider_builder_factory(daily, cfg):
    def builder(enabled=True):
        return ted.make_ted_stress_decider(daily, WEEKLY_DEPOSIT, enabled=enabled, **cfg)
    return builder


def run_sanity_checks(dev):
    """Pre-grid sanity checks (prereg.md): (1) non-degeneracy -- the
    primary config's stressed condition fires non-trivially on every core
    asset; (2) known-episode check -- the Aug 2007-Q1 2008 TED-spread
    onset must read mostly stressed, strictly within dev dates."""
    cfg = ted.PRIMARY_CONFIG
    rows = []
    for asset in ASSETS:
        daily = dev["prices"][asset]
        stressed = ted.compute_stressed(daily, cfg["lookback_years"], cfg["stress_pctile"], cfg["persistence_days"])
        frac_stressed = float(stressed.mean())
        rows.append({"asset": asset, "dev_days": len(stressed), "stressed_days": int(stressed.sum()), "frac_stressed": frac_stressed})

    # Known-episode check: Aug 2007 - Mar 2008 TED-spread onset, checked on
    # the signal itself (asset-agnostic), well within dev dates (pre-2020).
    stressed_at_signal_dates = ted._stressed_at_signal_dates(
        cfg["lookback_years"], cfg["stress_pctile"], cfg["persistence_days"]
    )
    raw = ted.fetch_raw_signal()
    window = stressed_at_signal_dates.loc["2007-08-01":"2008-03-31"]
    raw_window = raw.loc["2007-08-01":"2008-03-31"]
    episode = {
        "window": "2007-08-01..2008-03-31",
        "n_obs": int(len(window)),
        "frac_stressed_in_window": float(window.mean()) if len(window) else float("nan"),
        "ted_at_window_start": float(raw_window.iloc[0]) if len(raw_window) else float("nan"),
        "ted_at_window_end": float(raw_window.iloc[-1]) if len(raw_window) else float("nan"),
        "max_ted_in_window": float(raw_window.max()) if len(raw_window) else float("nan"),
    }
    return rows, episode
...
def run_impl_checks(dev):
    ...
    # so cash never going negative is the principled ceiling bound (family
    # 021's fix, reused verbatim -- no flat-percentage tolerance).
    ...
    # Macro-specific extra check (prereg.md): shortening the publication
    # lag to 0 days must change SOME historical regime readings vs the
    # documented 2-day lag -- proving the lag actually matters (not a
    # no-op) -- while every real backtest below uses the documented lag
    # only.
    stressed_documented = ted.compute_stressed(daily, cfg["lookback_years"], cfg["stress_pctile"], cfg["persistence_days"])
    stressed_zero_lag = ted.compute_stressed(daily, cfg["lookback_years"], cfg["stress_pctile"], cfg["persistence_days"], lag_override_days=0)
    frac_diff = float((stressed_documented != stressed_zero_lag).mean())
...
def run_one(daily, rf, cfg, fee):
    decide = ted.make_ted_stress_decider(daily, WEEKLY_DEPOSIT, **cfg)
...
def cfg_id_for(ci, cfg):
    return (f"cfg{ci:02d}_lb{cfg['lookback_years']}_pc{cfg['stress_pctile']}"
            f"_tf{cfg['stress_tilt_fraction']}_pd{cfg['persistence_days']}")
...
def main():
    ...
    for row in sanity:
        if not (0.02 <= row["frac_stressed"] <= 0.98):
            raise SystemExit(f"Sanity check FAILED for {row['asset']}: frac_stressed={row['frac_stressed']:.4f} "
                              "outside (2%, 98%) -- degenerate regime, stopping before grid.")
    if episode["n_obs"] == 0:
        raise SystemExit("Known-episode check FAILED: no Aug 2007-Mar 2008 TEDRATE observations found in dev data.")
    if episode["frac_stressed_in_window"] < 0.5:
        raise SystemExit(
            f"Known-episode check FAILED: the primary config reads only "
            f"{episode['frac_stressed_in_window']:.1%} of the Aug 2007-Mar 2008 TED-spread onset window as STRESSED; "
            "expected a clear majority -- signal construction does not match the well-known episode."
        )
    print(f"Known-episode check PASSED: Aug 2007-Mar 2008 TED-onset window reads "
          f"{episode['frac_stressed_in_window']:.1%} stressed, TED spread rose from "
          f"{episode['ted_at_window_start']:.3f} to {episode['ted_at_window_end']:.3f} "
          f"(max {episode['max_ted_in_window']:.3f}) in-window.")
    ...
    configs = ted.grid_configs()
    assert len(configs) <= 36
    assert ted.PRIMARY_CONFIG in configs
    ...
    primary_ci = configs.index(ted.PRIMARY_CONFIG)
    primary_cfg_id = cfg_id_for(primary_ci, ted.PRIMARY_CONFIG)
    primary_trial_id = f"new_050_{primary_cfg_id}"
    ...
    for asset in ASSETS:
        ...
        row = {"asset": asset}
        for fee in FEES:
            s, _ = run_one(daily, rf, ted.PRIMARY_CONFIG, fee)
            dca_s = dca_cache[asset][fee]
