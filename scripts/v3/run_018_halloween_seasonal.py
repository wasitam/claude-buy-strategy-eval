"""Run family 018 (Halloween/Sell-in-May seasonal deposit timing) end-to-end
on development data: pre-backtest non-degeneracy sanity check ->
implementation checks -> full grid -> trial logging -> DSR/N_eff -> assess.
Writes state/trials/new_018_*.csv, updates state/trial_counter.json, and
dumps results into families/018-halloween-seasonal/_run_output.json for the
results.md writer. Modeled directly on scripts/v3/run_007_day_of_week.py
and run_006_turn_of_month.py.
"""
...
from src.backtest.v3.strategies import halloween_seasonal as hs

ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
STATE_DIR = os.path.join(ROOT, "state")
TRIALS_DIR = os.path.join(STATE_DIR, "trials")
FAM_DIR = os.path.join(ROOT, "families", "018-halloween-seasonal")
FEES = [0.001, 0.0025]
...
def decider_builder_factory(daily, cfg):
    def builder(enabled=True):
        return hs.make_halloween_decider(daily, WEEKLY_DEPOSIT, enabled=enabled, **cfg)
    return builder


def pre_backtest_sanity_check(dev):
    """Required by the task before any grid runs: confirm the primary
    config's strong-season condition fires on a sensible ~50% of days for
    every asset (a fixed calendar split, so this should be trivially true,
    but must be CONFIRMED, not assumed -- catches an off-by-one
    month-boundary bug before any backtest is trusted)."""
    rows = []
    ok = True
    for asset in ASSETS:
        daily = dev["prices"][asset]
        is_strong = hs.compute_is_strong_season(
            daily, hs.PRIMARY_CONFIG["strong_season_start_month"], hs.PRIMARY_CONFIG["weak_season_start_month"]
        )
        n = len(is_strong)
        frac_strong = float(is_strong.sum()) / n
        # Sanity band: a 6-month/6-month split should land close to 50%,
        # generously bounded at (40%, 60%) to allow for unequal month
        # lengths and weekday/trading-day-count effects across a leap-year
        # calendar, without being so loose it would miss a real bug.
        row_ok = 0.40 <= frac_strong <= 0.60
        ok = ok and row_ok
        rows.append({"asset": asset, "n_days": n, "n_strong": int(is_strong.sum()), "frac_strong": frac_strong, "ok": row_ok})
        print(f"  {asset}: {is_strong.sum()}/{n} strong-season days ({frac_strong:.2%}) {'OK' if row_ok else 'FAIL'}")
    return ok, rows
...
def run_impl_checks(dev):
    ...
    rf = dev["rf"]
    builder = decider_builder_factory(daily, hs.PRIMARY_CONFIG)
    ...
    decide_hs = builder(enabled=True)
    res_hs = v3eng.run_single_asset(daily, WEEKLY_DEPOSIT, rf, decide_hs).to_frame()
    out["no_negative_cash_units_halloween"] = v3chk.check_no_negative_cash_or_units(res_hs)

    out["no_lookahead"] = v3chk.check_no_lookahead(daily, rf, builder, t_check=6000)
    out["point_in_time_macro"] = True  # price/calendar-only, no macro data -- vacuously satisfied

    # Extra check (prereg.md): total capital deployed never exceeds what the
    # SAME $500/week deposit stream would be worth if it had just sat in
    # cash earning IRX the whole time. Mirrors run_004/006/007's
    # capital-neutrality check exactly. This family never sells, so net
    # deployed == gross bought.
    cash_only = np.zeros(len(res_hs))
    rf_arr = rf.reindex(res_hs.index).fillna(0.0).to_numpy()
    dep_arr = res_hs["deposit"].to_numpy()
    c = 0.0
    for i in range(len(res_hs)):
        c *= 1.0 + rf_arr[i]
        c += dep_arr[i]
        cash_only[i] = c
    net_deployed = (res_hs["buy_usd"] - res_hs["sell_usd"]).cumsum().to_numpy()
...
def run_one(daily, rf, cfg, fee):
    decide = hs.make_halloween_decider(daily, WEEKLY_DEPOSIT, **cfg)
...
def cfg_id_for(ci, cfg):
    return (f"cfg{ci:02d}_ss{cfg['strong_season_start_month']}_ws{cfg['weak_season_start_month']}"
            f"_mt{cfg['mild_tilt_fraction']}_ml{cfg['max_lump_multiple']}")
...
def main():
    ...
    print("Pre-backtest non-degeneracy sanity check (primary config's strong-season frequency)...")
    sanity_ok, sanity_rows = pre_backtest_sanity_check(dev)
    if not sanity_ok:
        raise SystemExit("Pre-backtest sanity check FAILED -- strong-season frequency outside (40%,60%) band for at least one asset; stopping before any grid run.")
    ...
    primary_ci = configs.index(hs.PRIMARY_CONFIG)
    primary_cfg_id = cfg_id_for(primary_ci, hs.PRIMARY_CONFIG)
    primary_trial_id = f"new_018_{primary_cfg_id}"
    ...
    # SP500 (broad-market anchor, matches families 006/013/014/015/016's
    # own diagnostic choice).
    ...
    for asset in ASSETS:
        ...
        row = {"asset": asset}
        for fee in FEES:
            s, _ = run_one(daily, rf, hs.PRIMARY_CONFIG, fee)
            dca_s = dca_cache[asset][fee]
