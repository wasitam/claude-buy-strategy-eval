"""Run family 046 (VVIX vol-of-vol regime sizing) end-to-end on
development data: reachability/warm-up availability table -> distinction-
verification cross-tabs vs. families 016/025/041 -> implementation checks
-> pre-grid non-degeneracy sanity check -> full grid -> trial logging ->
DSR/N_eff -> assess. Writes state/trials/new_046_*.csv, updates
state/trial_counter.json, and dumps results into
families/046-vvix-regime-sizing/_run_output.json for the results.md
writer. Modeled directly on scripts/v3/run_041_vix_term_structure_carry.py's
structure.
"""
...
FAM_DIR = os.path.join(ROOT, "families", "046-vvix-regime-sizing")
FEES = [0.001, 0.0025]
WEEKLY_DEPOSIT = 500.0
ASSETS = ["SP500", "GOLD", "SILVER", "BTC", "OIL"]


def check_vvix_availability(dev) -> list[dict]:
    """Task-required: exact day counts of ^VVIX availability vs. each
    asset's own development window, computed live (not hand-copied from
    prereg.md)."""
    rows = []
    for asset in ASSETS:
        daily = dev["prices"][asset]
        vvix = vvs._aligned_vvix_close(daily.index)
        raw_vvix = v3data.fetch_yf_macro(vvs.VVIX_TICKER)
        n = len(daily)
        n_avail = int((daily.index >= raw_vvix.index.min()).sum())
        rows.append({
            "asset": asset, "dev_days": n, "vvix_available_days": n_avail,
            "frac_available": n_avail / n if n else float("nan"),
            "dev_start": str(daily.index.min().date()), "dev_end": str(daily.index.max().date()),
            "vvix_start": str(raw_vvix.index.min().date()),
        })
    return rows
...
def check_distinction_from_priors(dev) -> dict:
    ...
    family's elevated-VVIX flag (top-half of its own trailing percentile)
    against family 016's elevated-VIX-level flag, family 025's elevated-VRP
    flag, and family 041's backwardation flag, plus raw level correlations,
    on real overlapping SP500 development data (2007-01-03..2019-12-31,
    VVIX's own start)."""
    daily = dev["prices"]["SP500"]
    both_start = pd.Timestamp("2007-01-03")
    ...
    n = min(len(elevated_vvix), len(elevated_016), len(elevated_flag_025), len(backwardation_041))
    mask = (daily.index[:n] >= both_start)

    vix_close = vc._aligned_vix_close(daily.index)
    corr_vvix_vix = float(pd.Series(vvix_close.to_numpy()[:n][mask]).corr(pd.Series(vix_close.to_numpy()[:n][mask])))
    ...
    return {
        ...
        "corr_vvix_level_vs_vix_level": corr_vvix_vix,
        "vvix_not_a_relabeling_of_vix_level": bool(abs(corr_vvix_vix) < 0.9),
        "frac_flag_agreement_vs_016_elevated_vix": agreement(elevated_vvix, elevated_016),
        "frac_flag_agreement_vs_025_elevated_vrp": agreement(elevated_vvix, elevated_flag_025),
        "frac_flag_agreement_vs_041_backwardation": agreement(elevated_vvix, ~backwardation_041),
        "genuinely_different_from_016": bool(0.0 < agreement(elevated_vvix, elevated_016) < 1.0),
        "genuinely_different_from_025": bool(0.0 < agreement(elevated_vvix, elevated_flag_025) < 1.0),
        "genuinely_different_from_041": bool(0.0 < agreement(elevated_vvix, ~backwardation_041) < 1.0),
    }


def check_divergence_examples(dev) -> dict:
    """Task-required: concrete real-data example of VVIX/VIX divergence
    (moderate VIX, elevated VVIX) and a contrasting co-movement episode
    (both elevated together), on real pre-2020 dev data."""
    daily = dev["prices"]["SP500"]
    vvix_close = vvs._aligned_vvix_close(daily.index)
    vix_close = vc._aligned_vix_close(daily.index)
    pct_vvix = vvs.compute_percentile_rank(vvix_close, 252)
    idx = daily.index
    ...
    def row_at(date_str):
        ...
        return {
            ...
            "vvix": float(vvix_close.iloc[loc]),
            "vvix_pctile": float(pct_vvix[loc]),
        }

    return {
        "divergence_case_dec_2014": row_at("2014-12-10"),
        "comovement_case_2008_oct": row_at("2008-10-20"),
        "comovement_case_2008_nov": row_at("2008-11-24"),
    }


def decider_builder_factory(daily, vvix_lookback, k, min_mult, max_mult):
    def builder(enabled=True):
        return vvs.make_vvix_regime_decider(
            daily, WEEKLY_DEPOSIT, vvix_lookback=vvix_lookback, k=k, min_mult=min_mult, max_mult=max_mult,
            enabled=enabled,
        )
    return builder
...
def check_flat_k_zero_equals_dca(daily, rf) -> bool:
    ...
    flat_decide = vvs.make_vvix_regime_decider(
        daily, WEEKLY_DEPOSIT, vvix_lookback=252, k=0.0, min_mult=0.5, max_mult=2.0, enabled=True,
    )
...
def check_cash_reserve_dynamics(daily, rf) -> dict:
    cfg = vvs.PRIMARY_CONFIG
    decide = vvs.make_vvix_regime_decider(daily, WEEKLY_DEPOSIT, **cfg)
    ...
    m = vvs.compute_multiplier(daily, cfg["vvix_lookback"], cfg["k"], cfg["min_mult"], cfg["max_mult"])
    n = min(len(res), len(m))
    ...
    return {
        ...
        # For this family low_mult (elevated VVIX, risk-off) is the "banking" tier
        # and high_mult (depressed VVIX) is the "spending" tier -- opposite
        # naming convention from families 016/041 but same underlying check.
        "avg_cash_elevated_vvix_weeks_low_mult": avg_cash_low_mult,
        "avg_cash_depressed_vvix_weeks_high_mult": avg_cash_high_mult,
        "reserve_drawn_down_on_depressed_vvix_weeks": bool(avg_cash_high_mult < avg_cash_low_mult),
    }
...
def run_impl_checks(dev):
    ...
    daily = dev["prices"]["SP500"]
    rf = dev["rf"]
    builder = decider_builder_factory(daily, **vvs.PRIMARY_CONFIG)

    avail = check_vvix_availability(dev)
    out["vvix_availability"] = avail
    ...
    out["distinction_confirmed"] = (
        ...
        and dist_check["vvix_not_a_relabeling_of_vix_level"]
    )
    ...
    out["reserve_drawn_down_on_depressed_vvix_weeks"] = cash_check["reserve_drawn_down_on_depressed_vvix_weeks"]
    ...
    aggressive_cfg = {"vvix_lookback": 126, "k": 1.5, "min_mult": 0.25, "max_mult": 2.0}
    decide_agg = vvs.make_vvix_regime_decider(daily, WEEKLY_DEPOSIT, **aggressive_cfg, enabled=True)
...
def cfg_id_for(ci, cfg):
    return f"cfg{ci:02d}_vl{cfg['vvix_lookback']}_k{cfg['k']}_mn{cfg['min_mult']}_mx{cfg['max_mult']}"
...
def main():
    ...
    print("Checking ^VVIX availability vs. each asset's dev window...")
    avail = check_vvix_availability(dev)
    print(json.dumps(avail, indent=2))

    print("Running distinction-verification checks vs. families 016/025/041 + implementation checks...")
    impl_checks = run_impl_checks(dev)
    print(json.dumps(impl_checks, indent=2, default=float))
    required = ["distinction_confirmed", "reserve_builds_up_vs_dca", "reserve_drawn_down_on_depressed_vvix_weeks",
    ...
    configs = vvs.grid_configs()
    assert len(configs) <= 36
    assert vvs.PRIMARY_CONFIG in configs
    ...
    for ci, cfg in enumerate(configs):
        ...
        for asset in ASSETS:
            ...
            row = {"config_id": cfg_id, "asset": asset,
                   "vvix_lookback": cfg["vvix_lookback"], "k": cfg["k"],
    ...
    primary_ci = configs.index(vvs.PRIMARY_CONFIG)
    primary_cfg_id = cfg_id_for(primary_ci, vvs.PRIMARY_CONFIG)
    primary_trial_id = f"new_046_{primary_cfg_id}"
    ...
    if check_4_1["pass"]:
        print("Sec 4.1 PASSED -- run scripts/v3/robustness_046_vvix_regime_sizing.py next "
    ...
    for asset in ASSETS:
        ...
        row = {"asset": asset}
        for fee in FEES:
            s, _ = run_one(daily, rf, vvs.PRIMARY_CONFIG, fee)
            dca_s = dca_cache[asset][fee]
