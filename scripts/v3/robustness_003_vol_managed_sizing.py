"""Robustness diagnostics for family 003's primary configuration (plan sec
4.3). Modeled directly on scripts/v3/robustness_001_trend_exit.py (same
single-asset pattern): rolling windows (3y/5y for SP500/GOLD/SILVER/OIL, 2y
for BTC), block bootstrap raw+detrended, and placebo circular-shift -- all at
the reduced n_sims=120 precedent set by family 001/002 (state/bugfix_log.md,
families/001-trend-exit/results.md judgment call #3) since SP500's 92-year
daily history makes each full-history simulation multi-second.

Placebo signal here is the strategy's own continuous sizing multiplier m_t
(not a binary in/cash flag) -- circular-shifting m_t and rerunning tests
whether the specific temporal alignment of the vol-timing signal (not just
"some correlated-noise multiplier sequence") is doing the work.
"""
...
from src.backtest.v3.strategies import vol_managed_sizing as vms

ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
FAM_DIR = os.path.join(ROOT, "families", "003-vol-managed-sizing")
...
def strat_builder(daily):
    return vms.make_vol_managed_decider(daily, WEEKLY_DEPOSIT, **vms.PRIMARY_CONFIG)
...
def main():
    ...
    signal_raw = vms.compute_multiplier(daily, **vms.PRIMARY_CONFIG)
