"""Run family 016 (VIX contrarian fear-gauge sizing) end-to-end on
development data: implementation checks -> full grid -> trial logging ->
DSR/N_eff -> assess. Writes state/trials/new_016_*.csv, updates
state/trial_counter.json, and dumps results into
families/016-vix-contrarian/_run_output.json for the results.md writer.
Modeled directly on scripts/v3/run_015_dxy_regime.py.
"""
...
from src.backtest.v3.strategies import vix_contrarian as vxc

ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
STATE_DIR = os.path.join(ROOT, "state")
TRIALS_DIR = os.path.join(STATE_DIR, "trials")
FAM_DIR = os.path.join(ROOT, "families", "016-vix-contrarian")
FEES = [0.001, 0.0025]
...
# aggressive banking -- the config expected to spend cash fastest/hardest.
AGGRESSIVE_CORNER = {
    "vix_lookback": 252, "elevated_pct": 80.0, "buy_multiplier": 3.0,
    "calm_fraction": 0.75, "max_buy_multiple": vxc.MAX_BUY_MULTIPLE,
}


def decider_builder_factory(daily, cfg):
    def builder(enabled=True):
        return vxc.make_vix_contrarian_decider(daily, WEEKLY_DEPOSIT, enabled=enabled, **cfg)
    return builder
...
def run_impl_checks(dev):
    ...
    out["point_in_time_macro"] = True  # N/A: VIX is a market price series, no publication lag (see prereg.md)
...
def check_capital_never_exceeds_deposits(res: pd.DataFrame) -> bool:
    ...
    scripts/v3/run_015_dxy_regime.py's check of the same name."""
...
def run_one(daily, rf, cfg, fee):
    decide = vxc.make_vix_contrarian_decider(daily, WEEKLY_DEPOSIT, **cfg)
...
def cfg_id_for(ci, cfg):
    return (f"cfg{ci:02d}_lb{cfg['vix_lookback']}_ep{cfg['elevated_pct']}"
            f"_bm{cfg['buy_multiplier']}_cf{cfg['calm_fraction']}")
...
def main():
    ...
    _, res_primary_sp = run_one(daily_sp, rf, vxc.PRIMARY_CONFIG, 0.001)
    ...
    # config's elevated-fear regime frequency must be nontrivial, on EVERY
    # core asset (not just SP500), before we trust the grid run at all.
    frac_confirmed = {}
    for asset in ASSETS:
        daily_a = dev["prices"][asset]
        elevated_a = vxc.compute_elevated(daily_a, vxc.PRIMARY_CONFIG["vix_lookback"], vxc.PRIMARY_CONFIG["elevated_pct"])
        frac_confirmed[asset] = float(elevated_a.mean())
    impl_checks["primary_regime_nondegenerate"] = bool(all(0.02 < f < 0.5 for f in frac_confirmed.values()))
    impl_checks["primary_regime_frac_elevated_by_asset"] = frac_confirmed
    ...
    primary_ci = configs.index(vxc.PRIMARY_CONFIG)
    primary_cfg_id = cfg_id_for(primary_ci, vxc.PRIMARY_CONFIG)
    primary_trial_id = f"new_016_{primary_cfg_id}"
    ...
    for asset in ASSETS:
        ...
        row = {"asset": asset}
        for fee in FEES:
            s, _ = run_one(daily, rf, vxc.PRIMARY_CONFIG, fee)
            dca_s = dca_cache[asset][fee]
