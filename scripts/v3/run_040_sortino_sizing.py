"""Run family 040 (rolling Sortino-ratio sizing) end-to-end on development
data: downside-deviation formula spot-check -> Sortino-vs-Sharpe divergence
example (prereg.md's required distinction from family 031) -> implementation
checks -> pre-grid non-degeneracy sanity check -> full grid -> trial
logging -> DSR/N_eff -> assess. Writes state/trials/new_040_*.csv, updates
state/trial_counter.json, and dumps results into
families/040-sortino-sizing/_run_output.json for the results.md writer.
Modeled directly on scripts/v3/run_039_realized_kurtosis_sizing.py's
structure, adapted for this family's distinction from family 031 (Kelly-
Sharpe sizing) instead of family 039's own distinction from family 023.
"""
...
from src.backtest.v3.strategies import sortino_sizing as sos

ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
STATE_DIR = os.path.join(ROOT, "state")
TRIALS_DIR = os.path.join(STATE_DIR, "trials")
FAM_DIR = os.path.join(ROOT, "families", "040-sortino-sizing")
FEES = [0.001, 0.0025]
...
def _sharpe_ratio(r: np.ndarray) -> float:
    ...
    m = r.mean()
    std = r.std(ddof=1)
    return float(m / std) if std > 0 else float("nan")


def _sortino_ratio(r: np.ndarray) -> float:
    r = np.asarray(r, dtype=float)
    m = r.mean()
    dd = sos.compute_downside_deviation(r)
    return float(m / dd) if dd > 0 else float("nan")


def check_downside_deviation_negative_only() -> dict:
    """Task-required (b): spot-check that the downside-deviation
    computation only includes negative-return days, on a toy array with
    known values (verified live by the module under test)."""
    # 3 positive days (should contribute 0) + 2 negative days (should drive the whole statistic)
    toy = np.array([0.02, 0.05, 0.10, -0.03, -0.01])
    dd = sos.compute_downside_deviation(toy)
    expected = np.sqrt(np.mean(np.array([0.0, 0.0, 0.0, 0.03, 0.01]) ** 2))
    # Changing the positive days' magnitude must not move the downside deviation at all.
    toy_pos_changed = np.array([0.99, 1.50, 2.00, -0.03, -0.01])
    dd_pos_changed = sos.compute_downside_deviation(toy_pos_changed)
    return {
        "toy_window": toy.tolist(),
        "downside_deviation": dd,
        "expected": float(expected),
        "matches_expected": bool(np.isclose(dd, expected, atol=1e-12)),
        "invariant_to_positive_day_magnitude": bool(np.isclose(dd, dd_pos_changed, atol=1e-12)),
    }


def check_sortino_sharpe_divergence(dev) -> dict:
    """Task-required (c): the required rigorous distinction from family
    031, a concrete real-data window where Sortino and Sharpe ratios
    diverge meaningfully for the same asset/period. Computed live here
    (not hand-copied from prereg.md)."""
    ...
    target_date = pd.Timestamp("1964-02-20")
    i = idx.get_loc(target_date)
    window = log_ret[i - 59:i + 1]
    assert not np.isnan(window).any()

    sharpe = _sharpe_ratio(window)
    sortino = _sortino_ratio(window)
    dd = sos.compute_downside_deviation(window)
    std = window.std(ddof=1)

    return {
        "window": f"{idx[i - 59].date()}..{idx[i].date()}",
        "mean_return": float(window.mean()),
        "std_symmetric": float(std),
        "downside_deviation": float(dd),
        "n_positive_days": int((window > 0).sum()),
        "n_negative_days": int((window < 0).sum()),
        "max_up_day": float(window.max()),
        "max_down_day": float(window.min()),
        "sharpe_ratio": sharpe,
        "sortino_ratio": sortino,
        "ratio_sortino_over_sharpe": float(sortino / sharpe) if sharpe else float("nan"),
        "divergence_confirmed": bool(sortino / sharpe > 2.0 and std > 3.0 * dd),
    }


def decider_builder_factory(daily, sortino_window, pctile_lookback, k, min_mult):
    def builder(enabled=True):
        return sos.make_sortino_sizing_decider(
            daily, WEEKLY_DEPOSIT, sortino_window=sortino_window, pctile_lookback=pctile_lookback,
...
def check_flat_k_zero_equals_dca(daily, rf) -> bool:
    ...
    pattern, per family 020/033/034/035/039 precedent): a real k=0.0 grid-
    shaped code path (enabled=True, going through the actual Sortino/
    ...
    flat_decide = sos.make_sortino_sizing_decider(
        daily, WEEKLY_DEPOSIT, sortino_window=60, pctile_lookback=504, k=0.0, min_mult=0.5, enabled=True,
    )
...
def check_cash_reserve_dynamics(daily, rf) -> dict:
    """Task-required (e): with min_mult=0.5<1.0 in the primary config,
    confirm a genuine reserve is banked during depressed-Sortino-
    percentile weeks (average cash share higher than plain DCA) and drawn
    down during elevated-Sortino-percentile weeks."""
    cfg = sos.PRIMARY_CONFIG
    decide = sos.make_sortino_sizing_decider(daily, WEEKLY_DEPOSIT, **cfg)
    ...
    m = sos.compute_multiplier(daily, cfg["sortino_window"], cfg["pctile_lookback"], cfg["k"], cfg["min_mult"])
    n = min(len(res), len(m))
    low_mult_tier = m[:n] < 1.0   # depressed Sortino percentile -> under-invest -> should bank cash
    high_mult_tier = m[:n] > 1.0  # elevated Sortino percentile -> over-invest -> should draw cash down
...
def check_capital_never_exceeds_deposits(daily, rf, res: pd.DataFrame) -> bool:
    """Principled-bound capital-neutrality check (family 021's fix)."""
...
def run_impl_checks(dev):
    ...
    daily = dev["prices"]["SP500"]
    rf = dev["rf"]
    builder = decider_builder_factory(daily, **sos.PRIMARY_CONFIG)

    dd_check = check_downside_deviation_negative_only()
    out["downside_deviation_check"] = dd_check
    out["downside_deviation_correct"] = dd_check["matches_expected"] and dd_check["invariant_to_positive_day_magnitude"]

    div_check = check_sortino_sharpe_divergence(dev)
    out["sortino_sharpe_divergence"] = div_check
    ...
    aggressive_cfg = {"sortino_window": 40, "pctile_lookback": 504, "k": 1.5, "min_mult": 0.25}
    decide_agg = sos.make_sortino_sizing_decider(daily, WEEKLY_DEPOSIT, **aggressive_cfg, enabled=True)
...
def cfg_id_for(ci, cfg):
    return f"cfg{ci:02d}_sw{cfg['sortino_window']}_pl{cfg['pctile_lookback']}_k{cfg['k']}_mn{cfg['min_mult']}"
...
def main():
    ...
    print("Running downside-deviation spot-check + Sortino/Sharpe divergence + implementation checks...")
    impl_checks = run_impl_checks(dev)
    print(json.dumps(impl_checks, indent=2, default=float))
    required = ["downside_deviation_correct", "divergence_confirmed",
    ...
    configs = sos.grid_configs()
    assert len(configs) <= 36
    assert sos.PRIMARY_CONFIG in configs
    ...
    for ci, cfg in enumerate(configs):
        ...
        for asset in ASSETS:
            ...
            row = {"config_id": cfg_id, "asset": asset,
                   "sortino_window": cfg["sortino_window"], "pctile_lookback": cfg["pctile_lookback"],
    ...
    primary_ci = configs.index(sos.PRIMARY_CONFIG)
    primary_cfg_id = cfg_id_for(primary_ci, sos.PRIMARY_CONFIG)
    primary_trial_id = f"new_040_{primary_cfg_id}"
    ...
    if check_4_1["pass"]:
        print("Sec 4.1 PASSED -- run scripts/v3/robustness_040_sortino_sizing.py next "
    ...
    for asset in ASSETS:
        ...
        row = {"asset": asset}
        for fee in FEES:
            s, _ = run_one(daily, rf, sos.PRIMARY_CONFIG, fee)
            dca_s = dca_cache[asset][fee]
