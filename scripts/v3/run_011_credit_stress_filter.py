"""Run family 011 (credit-stress risk-off filter) end-to-end on development
data: implementation checks -> full grid -> trial logging -> DSR/N_eff ->
assess. Writes state/trials/new_011_*.csv, updates state/trial_counter.json,
and dumps results into families/011-credit-stress-filter/_run_output.json
for the results.md writer. Modeled directly on scripts/v3/run_006_turn_of_month.py
...
from src.backtest.v3.strategies import credit_stress_filter as csf

ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
STATE_DIR = os.path.join(ROOT, "state")
TRIALS_DIR = os.path.join(STATE_DIR, "trials")
FAM_DIR = os.path.join(ROOT, "families", "011-credit-stress-filter")
FEES = [0.001, 0.0025]
...
def decider_builder_factory(daily, cfg):
    def builder(enabled=True):
        return csf.make_credit_stress_decider(daily, WEEKLY_DEPOSIT, enabled=enabled, **cfg)
    return builder
...
def run_impl_checks(dev):
    ...
    # documented lag -- proving the lag actually matters (not a no-op) --
    # while every real backtest below uses the documented lag only.
    stressed_documented = csf.compute_stressed(daily, cfg["signal_choice"], cfg["lookback_years"], cfg["stress_pctile"])
    stressed_zero_lag = csf.compute_stressed(daily, cfg["signal_choice"], cfg["lookback_years"], cfg["stress_pctile"], lag_override_days=0)
...
def run_one(daily, rf, cfg, fee):
    decide = csf.make_credit_stress_decider(daily, WEEKLY_DEPOSIT, **cfg)
...
def cfg_id_for(ci, cfg):
    return (f"cfg{ci:02d}_{cfg['signal_choice']}_lb{cfg['lookback_years']}"
            f"_pc{cfg['stress_pctile']}_tf{cfg['stress_tilt_fraction']}")
...
def main():
    ...
    required = ["degenerate_equals_dca", "no_negative_cash_units_dca", "no_negative_cash_units_primary",
                "no_lookahead", "no_lookahead_late", "point_in_time_macro"]
    ...
    primary_ci = configs.index(csf.PRIMARY_CONFIG)
    primary_cfg_id = cfg_id_for(primary_ci, csf.PRIMARY_CONFIG)
    primary_trial_id = f"new_011_{primary_cfg_id}"
    ...
    for asset in ASSETS:
        ...
        row = {"asset": asset}
        for fee in FEES:
            s, _ = run_one(daily, rf, csf.PRIMARY_CONFIG, fee)
            dca_s = dca_cache[asset][fee]
