"""Run family 003 (volatility-managed sizing) end-to-end on development data:
implementation checks -> full grid -> trial logging -> DSR/N_eff -> assess.
Writes state/trials/new_003_*.csv, updates state/trial_counter.json, and
dumps results into families/003-vol-managed-sizing/_run_output.json for the
results.md writer. Modeled directly on scripts/v3/run_001_trend_exit.py
...
def decider_builder_factory(daily, vol_lookback_days, ref_lookback_days, min_mult, max_mult):
    def builder(enabled=True):
        return vms.make_vol_managed_decider(
            daily, WEEKLY_DEPOSIT, vol_lookback_days=vol_lookback_days, ref_lookback_days=ref_lookback_days,
            min_mult=min_mult, max_mult=max_mult, enabled=enabled,
        )
    return builder
...
def run_impl_checks(dev):
    ...
    rf = dev["rf"]
    builder = decider_builder_factory(daily, **vms.PRIMARY_CONFIG)
    ...
    decide_vms = builder(enabled=True)
    res_vms = v3eng.run_single_asset(daily, WEEKLY_DEPOSIT, rf, decide_vms).to_frame()
    out["no_negative_cash_units_vol_managed"] = v3chk.check_no_negative_cash_or_units(res_vms)
...
def run_one(daily, rf, cfg, fee):
    decide = vms.make_vol_managed_decider(daily, WEEKLY_DEPOSIT, **cfg)
...
def cfg_id_for(ci, cfg):
    return f"cfg{ci:02d}_v{cfg['vol_lookback_days']}_r{cfg['ref_lookback_days']}_lo{cfg['min_mult']}_hi{cfg['max_mult']}"
...
def main():
    ...
    primary_ci = configs.index(vms.PRIMARY_CONFIG)
    primary_cfg_id = cfg_id_for(primary_ci, vms.PRIMARY_CONFIG)
    primary_trial_id = f"new_003_{primary_cfg_id}"
