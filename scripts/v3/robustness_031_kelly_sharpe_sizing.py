"""Robustness diagnostics for family 031's primary configuration (plan sec
4.3). Modeled on scripts/v3/robustness_028_ma_crossover_regime.py: rolling
...
array m_t (derived from the percentile-ranked trailing Sharpe), NOT the
raw price series -- circular-shifting m_t tests whether the specific
temporal alignment of the Sharpe-percentile signal (not just "some
correlated sizing pattern of the same overall distribution") is doing the
work.
"""
...
def strat_builder(daily):
    return ksz.make_kelly_sharpe_decider(daily, WEEKLY_DEPOSIT, **ksz.PRIMARY_CONFIG)
...
def main():
    ...
    m_real = ksz.compute_multiplier(daily, **cfg)
    ...
    def strategy_decide_from_signal(shifted_m):
        lump_cap = ksz.MAX_LUMP_MULTIPLE * WEEKLY_DEPOSIT
