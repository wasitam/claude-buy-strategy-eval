"""Robustness diagnostics for family 028's primary configuration (plan sec
4.3). Modeled on scripts/v3/robustness_021_amihud_illiquidity.py /
robustness_027... pattern: rolling windows (3y/5y for SP500/GOLD/SILVER/
OIL, 2y for BTC), block bootstrap raw+detrended, and placebo
circular-shift -- at the n_sims=60 time-budget convention, run because
sec 4.1 passed on the primary config, per this iteration's task
instruction ("sec 4.3 in full if sec 4.1 passes, time-boxed n_sims~60").

Placebo signal here is the primary config's own confirmed-bullish
(golden-cross regime) boolean array, NOT the raw price series -- circular-
shifting the confirmed regime (which the weekly decider reads) tests
whether the specific temporal alignment of the crossover signal (not just
"some correlated bull/bear regime of the same overall frequency") is
doing the work, mirroring family 021's own placebo design.
"""
...
def strat_builder(daily):
    return mac.make_ma_crossover_decider(daily, WEEKLY_DEPOSIT, **mac.PRIMARY_CONFIG)
...
def main():
    ...
    # --- placebo: circular-shift the primary config's own confirmed-bullish array, SP500 ---
    cfg = mac.PRIMARY_CONFIG
    confirmed = mac.compute_confirmed_bullish(daily, cfg["fast_days"], cfg["slow_days"], cfg["persistence_days"])
    ...
    def strategy_decide_from_signal(shifted_confirmed):
        shifted_confirmed = shifted_confirmed.astype(bool)
        m = np.where(shifted_confirmed, cfg["bull_mult"], cfg["bear_mult"]).astype(float)

        def decide(t, cash):
            target_buy_usd = WEEKLY_DEPOSIT * float(m[t])
            capped_buy_usd = min(target_buy_usd, cfg["max_lump_multiple"] * WEEKLY_DEPOSIT if "max_lump_multiple" in cfg else 6.0 * WEEKLY_DEPOSIT)
            return capped_buy_usd, 0.0, {"multiplier": float(m[t])}

        return decide

    df_pl, pct_w, pct_s = v3rob.placebo_circular_shift(
        daily, rf, WEEKLY_DEPOSIT, confirmed.astype(float), strategy_decide_from_signal, bench_builder,
