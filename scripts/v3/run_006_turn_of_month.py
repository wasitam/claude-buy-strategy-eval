"""Run family 006 (turn-of-month deposit timing) end-to-end on development
data: implementation checks -> full grid -> trial logging -> DSR/N_eff ->
assess. Writes state/trials/new_006_*.csv, updates state/trial_counter.json,
and dumps results into families/006-turn-of-month/_run_output.json for the
results.md writer. Modeled directly on scripts/v3/run_005_tsmom_sizing.py
(same single-asset, no-sell, weekly-decision-cadence pattern), plus an
extra capital-neutrality check modeled on run_004_value_averaging.py.
"""
...
FAM_DIR = os.path.join(ROOT, "families", "006-turn-of-month")
FEES = [0.001, 0.0025]
WEEKLY_DEPOSIT = 500.0
ASSETS = ["SP500", "GOLD", "SILVER", "BTC", "OIL"]


def decider_builder_factory(daily, days_before_month_end, days_after_month_start, mild_tilt_fraction, max_lump_multiple):
    def builder(enabled=True):
        return tom.make_tom_decider(
            daily, WEEKLY_DEPOSIT, days_before_month_end=days_before_month_end,
            days_after_month_start=days_after_month_start, mild_tilt_fraction=mild_tilt_fraction,
            max_lump_multiple=max_lump_multiple, enabled=enabled,
        )
    return builder
...
def run_impl_checks(dev):
    ...
    rf = dev["rf"]
    builder = decider_builder_factory(daily, **tom.PRIMARY_CONFIG)
    ...
    decide_tom = builder(enabled=True)
    res_tom = v3eng.run_single_asset(daily, WEEKLY_DEPOSIT, rf, decide_tom).to_frame()
    out["no_negative_cash_units_tom"] = v3chk.check_no_negative_cash_or_units(res_tom)
    ...
    # cash earning IRX the whole time -- i.e. every dollar this family buys
    # traces back to this account's own deposits + the interest they
    # themselves earned while un-invested (never external capital, never
    # leverage). Mirrors run_004_value_averaging.py's capital-neutrality
    # check exactly. This family never sells, so net deployed == gross
    # bought.
    cash_only = np.zeros(len(res_tom))
    rf_arr = rf.reindex(res_tom.index).fillna(0.0).to_numpy()
    dep_arr = res_tom["deposit"].to_numpy()
    c = 0.0
    for i in range(len(res_tom)):
        c *= 1.0 + rf_arr[i]
        c += dep_arr[i]
        cash_only[i] = c
    net_deployed = (res_tom["buy_usd"] - res_tom["sell_usd"]).cumsum().to_numpy()
...
def run_one(daily, rf, cfg, fee):
    decide = tom.make_tom_decider(daily, WEEKLY_DEPOSIT, **cfg)
...
def cfg_id_for(ci, cfg):
    return (f"cfg{ci:02d}_db{cfg['days_before_month_end']}_da{cfg['days_after_month_start']}"
...
def main():
    ...
    primary_ci = configs.index(tom.PRIMARY_CONFIG)
    primary_cfg_id = cfg_id_for(primary_ci, tom.PRIMARY_CONFIG)
    primary_trial_id = f"new_006_{primary_cfg_id}"
    ...
    for asset in ASSETS:
        ...
        row = {"asset": asset}
        for fee in FEES:
            s, _ = run_one(daily, rf, tom.PRIMARY_CONFIG, fee)
            dca_s = dca_cache[asset][fee]
