"""Run family 041 (VIX futures term-structure carry) end-to-end on
development data: reachability/warm-up availability table -> distinction-
verification cross-tab vs. family 016 -> implementation checks -> pre-grid
non-degeneracy sanity check -> full grid -> trial logging -> DSR/N_eff ->
assess. Writes state/trials/new_041_*.csv, updates state/trial_counter.json,
and dumps results into families/041-vix-term-structure-carry/_run_output.json
for the results.md writer. Modeled directly on
scripts/v3/run_040_sortino_sizing.py's structure.
"""
...
def check_vix3m_availability(dev) -> list[dict]:
    """Task-required: exact day counts of ^VIX3M availability vs. each
    asset's own development window, computed live (not hand-copied from
    prereg.md)."""
    rows = []
    for asset in ASSETS:
        daily = dev["prices"][asset]
        ratio = vtc.compute_ratio(daily.index)
        n = len(daily)
        n_avail = int(ratio.notna().sum())
        rows.append({
            "asset": asset, "dev_days": n, "vix3m_available_days": n_avail,
...
def check_distinction_from_family_016(dev) -> dict:
    """Task-required distinction-verification check: cross-tabulate this
    family's backwardation flag (ratio_t < 1) against family 016's own
    primary-config elevated-VIX-level flag, and the raw correlation
    between the VIX3M/VIX ratio and the raw VIX level, on real overlapping
    development data (SP500, 2006-07-17..2019-12-31)."""
    daily = dev["prices"]["SP500"]
    ratio = vtc.compute_ratio(daily.index)
    backwardation = (ratio < 1.0).to_numpy()
    ...
    n = min(len(backwardation), len(elevated_016))

    both_start = pd.Timestamp("2006-07-17")
    mask = (daily.index[:n] >= both_start) & ratio.iloc[:n].notna().to_numpy()
    bw = backwardation[:n][mask]
    el = elevated_016[:n][mask]

    agreement = float((bw == el).mean()) if mask.sum() else float("nan")

    vix_close = vtc._aligned_close(vtc.VIX_TICKER, daily.index)
    corr = float(pd.Series(ratio.to_numpy()).corr(pd.Series(vix_close.to_numpy())))

    return {
        "n_overlap_days": int(mask.sum()),
        "frac_flag_agreement_backwardation_vs_016_elevated": agreement,
        "genuinely_different": bool(0.0 < agreement < 1.0),
        "corr_ratio_vs_raw_vix_level": corr,
        "ratio_not_a_relabeling_of_vix_level": bool(abs(corr) < 0.9),
    }


def decider_builder_factory(daily, ts_lookback, k, min_mult, max_mult):
    def builder(enabled=True):
        return vtc.make_term_structure_carry_decider(
            daily, WEEKLY_DEPOSIT, ts_lookback=ts_lookback, k=k, min_mult=min_mult, max_mult=max_mult,
            enabled=enabled,
        )
    return builder
...
def check_flat_k_zero_equals_dca(daily, rf) -> bool:
    ...
    pattern, per family 020/033/034/035/039/040 precedent): a real k=0.0
    grid-shaped code path (enabled=True, going through the actual
    ratio/percentile computation) must also reproduce plain DCA
    bit-for-bit."""
    ...
    flat_decide = vtc.make_term_structure_carry_decider(
        daily, WEEKLY_DEPOSIT, ts_lookback=252, k=0.0, min_mult=0.5, max_mult=2.0, enabled=True,
    )
...
def check_cash_reserve_dynamics(daily, rf) -> dict:
    ...
    decide = vtc.make_term_structure_carry_decider(daily, WEEKLY_DEPOSIT, **cfg)
    ...
    m = vtc.compute_multiplier(daily, cfg["ts_lookback"], cfg["k"], cfg["min_mult"], cfg["max_mult"])
    n = min(len(res), len(m))
...
def run_impl_checks(dev):
    ...
    daily = dev["prices"]["SP500"]
    rf = dev["rf"]
    builder = decider_builder_factory(daily, **vtc.PRIMARY_CONFIG)

    avail = check_vix3m_availability(dev)
    out["vix3m_availability"] = avail

    dist_check = check_distinction_from_family_016(dev)
    out["distinction_from_016"] = dist_check
    out["distinction_confirmed"] = dist_check["genuinely_different"] and dist_check["ratio_not_a_relabeling_of_vix_level"]
    ...
    aggressive_cfg = {"ts_lookback": 126, "k": 1.5, "min_mult": 0.25, "max_mult": 2.0}
    decide_agg = vtc.make_term_structure_carry_decider(daily, WEEKLY_DEPOSIT, **aggressive_cfg, enabled=True)
...
def cfg_id_for(ci, cfg):
    return f"cfg{ci:02d}_tl{cfg['ts_lookback']}_k{cfg['k']}_mn{cfg['min_mult']}_mx{cfg['max_mult']}"
...
def main():
    ...
    print("Checking ^VIX3M availability vs. each asset's dev window...")
    avail = check_vix3m_availability(dev)
    print(json.dumps(avail, indent=2))

    print("Running distinction-verification check vs. family 016 + implementation checks...")
    impl_checks = run_impl_checks(dev)
    print(json.dumps(impl_checks, indent=2, default=float))
    required = ["distinction_confirmed", "reserve_builds_up_vs_dca", "reserve_drawn_down_on_high_mult_weeks",
    ...
    configs = vtc.grid_configs()
    assert len(configs) <= 36
    assert vtc.PRIMARY_CONFIG in configs
    ...
    for ci, cfg in enumerate(configs):
        ...
        for asset in ASSETS:
            ...
            row = {"config_id": cfg_id, "asset": asset,
                   "ts_lookback": cfg["ts_lookback"], "k": cfg["k"],
    ...
    primary_ci = configs.index(vtc.PRIMARY_CONFIG)
    primary_cfg_id = cfg_id_for(primary_ci, vtc.PRIMARY_CONFIG)
    primary_trial_id = f"new_041_{primary_cfg_id}"
    ...
    if check_4_1["pass"]:
        print("Sec 4.1 PASSED -- run scripts/v3/robustness_041_vix_term_structure_carry.py next "
    ...
    for asset in ASSETS:
        ...
        row = {"asset": asset}
        for fee in FEES:
            s, _ = run_one(daily, rf, vtc.PRIMARY_CONFIG, fee)
            dca_s = dca_cache[asset][fee]
