"""Run family 001 (trend exit) end-to-end on development data:
implementation checks -> full grid -> trial logging -> DSR/N_eff -> assess.
Writes state/trials/new_001_*.csv, updates state/trial_counter.json, and
dumps results into families/001-trend-exit/_run_output.json for the
results.md writer.
"""
...
def decider_builder_factory(daily, sma_days, confirm_days, sell_on_exit):
    def builder(enabled=True):
        return te.make_trend_exit_decider(
            daily, WEEKLY_DEPOSIT, sma_days=sma_days, confirm_days=confirm_days,
            sell_on_exit=sell_on_exit, enabled=enabled,
        )
    return builder
...
def run_impl_checks(dev):
    ...
    rf = dev["rf"]
    builder = decider_builder_factory(daily, **te.PRIMARY_CONFIG)
    ...
    decide_te = builder(enabled=True)
    res_te = v3eng.run_single_asset(daily, WEEKLY_DEPOSIT, rf, decide_te).to_frame()
    out["no_negative_cash_units_trend"] = v3chk.check_no_negative_cash_or_units(res_te)
...
def run_one(daily, rf, cfg, fee):
    decide = te.make_trend_exit_decider(daily, WEEKLY_DEPOSIT, **cfg)
...
def main():
    ...
    configs = te.grid_configs()
    assert len(configs) <= 36
    print(f"Grid: {len(configs)} configs")

    # DCA baselines per asset per fee (needed for excess-return series & win checks)
    dca_cache = {}
    for asset in ASSETS:
    ...
    grid_rows = []
    trial_series = {}  # trial_id -> pooled excess-return series (for N_eff)
    per_config_asset_weekly = {}  # (cfg_id, asset, fee) -> weekly_returns series, for grid pass-rate & CSCV

    new_trial_count = 0
    for ci, cfg in enumerate(configs):
        cfg_id = f"cfg{ci:02d}_sma{cfg['sma_days']}_cd{cfg['confirm_days']}_sx{cfg['sell_on_exit']}"
        per_asset_excess = {}
    ...
    # --- Trial counter update (idempotent: a bugfix rerun of the SAME grid must
    # not double-count trials already logged -- see state/bugfix_log.md) ---
    ...
    primary_cfg_id = f"cfg{configs.index(te.PRIMARY_CONFIG):02d}_sma{te.PRIMARY_CONFIG['sma_days']}_cd{te.PRIMARY_CONFIG['confirm_days']}_sx{te.PRIMARY_CONFIG['sell_on_exit']}"
    primary_trial_id = f"new_001_{primary_cfg_id}"
    ...
    # --- sec 4.4: >=2/3 of grid beats DCA (wealth+sharpe) at 0.1% fee, pooled=majority-of-assets rule per config ---
    ...
    # CSCV PBO diagnostic, pooled SP500 weekly strategy returns across the grid (asset with longest, most liquid history)
