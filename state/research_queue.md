...
| 46 | Cross-asset average-correlation regime sizing: for each of the 5 core assets, compute the asset's own trailing pairwise return correlation against the OTHER 4 core assets (average of the 4 pairwise correlations, trailing window), a market-wide systemic-risk/co-movement proxy computed purely from already-loaded price data (no external data dependency, unlike the credit-spread/VIX-based regime families 011/016/019/027/042). Scale buy size UP when average trailing correlation sits LOW in its own trailing percentile (a diversification-rich, idiosyncratic-risk-dominated regime, historically associated with calmer, less systemically-stressed markets per Longin & Solnik's correlation-regime literature), scale DOWN when average correlation sits HIGH (a "correlations go to 1" systemic-stress regime, historically associated with crisis episodes across asset classes). Genuinely different mechanism from every prior family: it is the first signal in this loop keyed on a CROSS-asset statistic (each asset's OWN buy decision depends on its co-movement with the other 4 assets, not on any single-asset trailing return/volatility/higher-moment statistic alone), distinct from family 044's cross-asset ROTATION (which reallocates the marginal deposit across assets; this family never moves capital between assets, only resizes each asset's own weekly buy using a cross-asset-derived signal) and from every VIX/credit/macro regime family (011/016/019/025/027/032/042), which use an external, non-price-derived data series rather than the core assets' own realized co-movement. Must construct a concrete real-data example (e.g. a well-documented crisis window where average correlation spikes) verifying the statistic behaves as the mechanism requires. | Regime switch (macro / credit / sentiment) | Longin, F. and Solnik, B. (2001), "Extreme Correlation of International Equity Markets," *Journal of Finance* 56(2), 649-676 (correlation-regime/crisis co-movement literature); Ang, A. and Bekaert, G. (2002), "International Asset Allocation with Regime Shifts," *Review of Financial Studies* 15(4), 1137-1187 |
